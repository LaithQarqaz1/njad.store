...
(function(){
      ...
      // حارس خمول الشبكة (2026-07-21 «اللودر يتأخر ليختفي بأكثر من مكان»):
      // إخفاء اللودر مرهون بجبل أعلام حجز (عدّاد الكاتلوج + keepalive يفرض
      // الظهور كل 250ms، وأعلام __CATALOG/LEVELS_INLINE_LOADING__ تُقلب يدوياً
      // في مواضع متفرقة) — أي مسار ينسى التحرير يعلّق اللودر فوق محتوى مرسوم،
      // وforceHideStartupLoader كانت بلا أي مستدعٍ. هذا الحارس لا يخمّن:
      // يعتمد عدّاد طلبات API الحقيقي من غلاف fetch في site-core — لودر ظاهر
      // ≥5 ثوانٍ متصلة بينما لا طلب معلّقاً ولا استقر طلب منذ ≥3.5 ثانية =
      // حاجز مسرّب قطعاً، فيصفّر الحواجز يدوياً ثم يخفي، ويطبع أيّها تسرّب.
      (function startNetworkIdleLoaderWatchdog(){
        var visibleSinceMs = 0;
        function isPreloaderShown(){
          try {
            var el = document.getElementById("preloader");
            if (!el) return false;
            if (el.classList.contains("hidden") || el.classList.contains("closing")) return false;
        ...
        try {
          setInterval(function(){
            try {
              if (!isPreloaderShown()) { visibleSinceMs = 0; return; }
              var now = Date.now();
              if (!visibleSinceMs) { visibleSinceMs = now; return; }
              if ((now - visibleSinceMs) < 5000) return;
              if (Number(window.__API_FETCH_PENDING_COUNT__ || 0) > 0) return;
              var lastSettled = Number(window.__API_FETCH_LAST_SETTLED_AT__ || 0);
              if (lastSettled && (now - lastSettled) < 3500) return;
              var leakSnapshot = {
                catalogCount: Number(window.__CATALOG_PAGE_LOADER_ACTIVE_COUNT__ || 0),
                catalogInline: window.__CATALOG_INLINE_LOADING__ === true,
                levelsInline: window.__LEVELS_INLINE_LOADING__ === true,
                loaderHold: window.__LOADER_HOLD_ACTIVE__ === true,
                walletRoutePending: !!(document.documentElement && document.documentElement.classList.contains("inline-wallet-route-pending"))
              };
              window.__CATALOG_PAGE_LOADER_ACTIVE_COUNT__ = 0;
              window.__CATALOG_INLINE_LOADING__ = false;
              window.__LEVELS_INLINE_LOADING__ = false;
              try {
              ...
              removeStartupClass(document.documentElement, "catalog-loader-pending");
              removeStartupClass(document.documentElement, "levels-loader-pending");
              if (document.body) {
                removeStartupClass(document.body, "catalog-loader-pending");
                removeStartupClass(document.body, "levels-loader-pending");
              }
              forceHideStartupLoader("net-idle-watchdog");
              visibleSinceMs = 0;
              try { console.info("[loader-watchdog] cleared a stuck loader", leakSnapshot); } catch (_) {}
            } catch (_) {}
          }, 1000);
        } catch (_) {}
      })();
...
(function(){
      ...
      // The brand comes from site_settings.js \u2014 GENERATED from store.identity.json
      // (or the repo variables that override it). NEVER hardcode a store name
      // here: this template is deployed for several different stores, and a
      // literal left behind is exactly how the previous store's name leaks into
      // the tab title / installed-app name of the next one.
      var DEFAULT_STATIC_STORE_NAME = (function(){
        try {
          return normalizeText(window.__getSiteSetting ? window.__getSiteSetting("brand.storeName", "") : "");
        } catch (_) {
          return "";
        }
      })();
      ...
      function buildDynamicManifestPayload(){
        ...
        var shortName = normalizeText(name).slice(0, 24) || DEFAULT_STATIC_STORE_NAME;
