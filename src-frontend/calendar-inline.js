...
function openCalendar(){
  ...
  if (!SELECTED_DATE_STR || SELECTED_DATE_STR === "__all__" || SELECTED_DATE_STR < minDateStr) {
...
// الجلسة صارت D1 (sessionKeyInfo/postLoginPayload) وفايربيس لم تعد مصدر
// الحقيقة: مستخدم D1-only لا يملك firebase.auth.currentUser أبدًا، فيُعتمد
// سياق الجلسة المخزّن — نفس ما تفعله المحفظة في resolveWalletLoadUser.
function resolveOrdersUserFromSession(){
  try {
    const session = readOrdersSessionContext("");
    const uid = String(session && session.uid || "").trim();
    const sessionKey = String(session && session.sessionKey || "").trim();
    if (uid && sessionKey) return { uid };
  } catch (_) {}
  return null;
}
async function resolveOrdersCurrentUserForView(){
  ...
  if (user) return user;
  const sessionUser = resolveOrdersUserFromSession();
...
function buildProviderCheckUrl({ orderUuid = "", orderId = "", useruid = "", sessionKey = "" } = {}) {
  ...
  try {
    ...
    if (uid) query.set("useruid", uid);
    if (uuid) query.set("order_uuid", uuid);
    else if (id) query.set("order_id", id);
...
function readOrdersSessionContext(uid) {
  ...
  try {
    const bundleKey = String(window.__AUTH_SESSION_BUNDLE_STORAGE_KEY__ || "auth:session:bundle:v1");
    const bundle = readOrdersJsonStorage(bundleKey);
...
async function buildOrdersServerAuthContext(uid, { forceToken = false } = {}) {
  const wantedUid = String(uid || "").trim();
  const user = getOrdersCurrentUser() || await waitForOrdersCurrentUser(800);
  const userUid = String((user && user.uid) || "").trim();
  const session = readOrdersSessionContext(wantedUid || userUid);
  const resolvedUid = String(wantedUid || session.uid || userUid || "").trim();
  let idToken = "";
  if (user && typeof user.getIdToken === "function") {
    try {
      ...
      if (!forceToken) {
        try { idToken = String(await user.getIdToken(true) || "").trim(); } catch (_) {}
      }
    }
  }
  return {
...
function isOrdersSessionRejectedPayload(data, res) {
  const code = String(data?.error_code || data?.errorCode || data?.code || "").trim().toLowerCase();
  if (code === "131" || code === "132" || code === "session_missing" || code === "session_invalid" || code === "session_expired") return true;
  const text = String(data?.message || data?.error || "").trim();
  return Number(res?.status || 0) === 401 && /invalid session key|session key is required|session_missing|session_invalid|session_expired|رمز الجلسة|الجلسة/i.test(text);
}

async function refreshOrdersSessionOnce(authContext) {
  try {
    const user = authContext?.user || getOrdersCurrentUser() || await waitForOrdersCurrentUser(800);
    if (!user || typeof user.getIdToken !== "function") return null;
    if (typeof window.__syncCatalogAuthFromToken !== "function") return null;
    const freshToken = String(await user.getIdToken(true) || "").trim();
    if (!freshToken) return null;
    const basePayload = {
      ...(readOrdersJsonStorage("postLoginPayload") || {}),
      uid: String(authContext?.uid || user.uid || "").trim(),
      idToken: freshToken,
      token: freshToken,
      sessionKey: String(authContext?.sessionKey || "").trim(),
      session_key: String(authContext?.sessionKey || "").trim(),
      deviceId: String(authContext?.deviceId || "").trim()
    };
    const synced = await window.__syncCatalogAuthFromToken(freshToken, basePayload);
    if (!synced || !synced.sessionKey) return null;
    return buildOrdersServerAuthContext(String(synced.uid || basePayload.uid || ""), { forceToken: false });
  } catch (_) {
    return null;
  }
}
...
async function fetchOrdersFromServerRaw(uid, opts = {}) {
  ...
  if (!authContext.sessionKey) {
    const err = new Error("رمز الجلسة غير متوفر. سجّل الدخول مرة أخرى ثم أعد المحاولة.");
    err.status = 401;
    err.code = "session_missing";
    throw err;
  }
  const send = async (nextAuthContext) => {
    const requestUid = String(nextAuthContext?.uid || safeUid).trim();
    const requestSessionKey = String(nextAuthContext?.sessionKey || "").trim();
    const params = { useruid: requestUid };
    if (requestSessionKey) params.sessionKey = requestSessionKey;
  ...
  if ((!res.ok || data?.success === false || data?.ok === false) && isOrdersSessionRejectedPayload(data, res)) {
    const refreshed = await refreshOrdersSessionOnce(authContext);
...
function isUnsafeJsonIntegerText(value) {
  ...
  const normalizedToken = text[0] === "-" ? text.slice(1) : text;
  if (normalizedToken.length < 16 || typeof BigInt !== "function") return false;
  try {
    const parsedInteger = BigInt(text);
    const absoluteInteger = parsedInteger < 0 ? -parsedInteger : parsedInteger;
    return absoluteInteger > BigInt(Number.MAX_SAFE_INTEGER);
  } catch (_) {
    return false;
  }
}
...
function quoteUnsafeJsonIntegerLiterals(text) {
  ...
  while (i < source.length) {
    ...
    const startsNumber = (ch >= "0" && ch <= "9") || (ch === "-" && source[i + 1] >= "0" && source[i + 1] <= "9");
    if (!startsNumber) {
      out += ch;
      i += 1;
      continue;
...
function safeJsonParse(str, fallback = null){
  ...
  try { return parseJsonPreservingUnsafeIntegers(str); } catch { return fallback; }
}
...
function redactProviderUserText(raw) {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object") throw new Error("not_json_object");
...
function isOrderCatalogDescriptorReply(value, order) {
  ...
  const candidates = [
    current.categoryName, current.category_name, current.category,
    current.gameName, current.game_name, current.gameLabel, current.game_label, current.game, current.gameSlug, current.game_slug,
    current.title, current.productTitle, current.productName, current.serviceName, current.name,
    pub.categoryName, pub.category_name, pub.category,
    pub.gameName, pub.game_name, pub.gameLabel, pub.game_label, pub.game, pub.gameSlug, pub.game_slug,
    pub.title, pub.productTitle, pub.productName, pub.serviceName, pub.name,
    priv.categoryName, priv.category_name, priv.category,
    priv.gameName, priv.game_name, priv.gameLabel, priv.game_label, priv.game, priv.gameSlug, priv.game_slug,
    priv.title, priv.productTitle, priv.productName, priv.serviceName, priv.name,
...
function resolveOrderReplyCandidate(value, order) {
  ...
  if (!cleaned) return "";
  return isOrderCatalogDescriptorReply(cleaned, order) ? "" : cleaned;
}
...
function splitOffersText(raw){
  ...
  if (!text) return [];
  let parts = text.split("أ¢â‚¬آ¢").map((p) => p.trim()).filter(Boolean);
...
function formatOffersHtml(order){
  ...
  const items = parts.map((part) => `<li class="offer-item"><div class="offer-name">${escapeHtml(part)}</div></li>`).join("");
  return `<ul class="offers-list">${items}</ul>`;
}
...
function getCurrencyContext(){
  ...
  if (selected) selected = String(selected).trim();
...
function normalizeCurrencyCode(value, rates){
  ...
  for (const key of Object.keys(rates)) {
    ...
    if (code && rawLower === code.toLowerCase()) {
      const isBaseUsd = code === "USD" && (id === "1" || String(key) === "1");
      if (isBaseUsd) return key;
      if (!firstCodeKey) firstCodeKey = key;
    }
    if (symbol && raw === symbol) return key;
    if (nameAr && raw === nameAr) return key;
    if (name && rawLower === name.toLowerCase()) return key;
  }
...
function formatAmountDisplay(totalStr, total, currency){
  ...
  if (fallback === "-") return "-";
  if (/[A-Z]{3,8}/.test(fallback) || /[$أ¢â€ڑآ¬آآ£]|ر\.س|د\.إ|د\.ظئ’|ر\.ق|د\.ب|ر\.ع|د\.ا/.test(fallback)) return fallback;
...
function formatStatusLabel(value){
  ...
  if (raw === 'تم_الشحن' || raw === 'تم الشحن') return ordersT("orders.status.accepted", "مقبول");
  const normalized = raw.toLowerCase();
  if (normalized.includes('مكتمل') || normalized === 'completed' || normalized === 'success') {
    return ordersT("orders.status.completed", "مكتمل");
  }
  if (normalized === 'partial') return ordersT("orders.status.partial", "مكتمل جزئياً");
  if (normalized.includes('ملغي') || normalized === 'canceled' || normalized === 'cancelled') {
    return ordersT("orders.status.cancelled", "ملغي");
  }
  if (normalized.includes('مرفوض') || normalized.startsWith('reject') || normalized === 'failed' || normalized === 'fail') {
    return ordersT("orders.status.rejected", "مرفوض");
  }
  if (
  ...
  ) return ordersT("orders.status.inProgress", "قيد التنفيذ");
  return raw;
}
...
function parseOrderTimeCandidate(value){
  try {
    ...
    if (typeof value === "object" && value.seconds) return (Number(value.seconds) * 1000) | 0;
...
function formatOrderExecutionDuration(order){
  ...
  if (Number.isFinite(directMs) && directMs > 0) return formatOrderDurationMs(directMs);
  const startMs = getOrderTimeMs(order);
  const endMs = getOrderStatusUpdatedTimeMs(order);
  if (!startMs || !endMs || endMs <= startMs) return "";
  return formatOrderDurationMs(endMs - startMs);
}
...
function orderMatchesSearch(o, qNorm){
  ...
  const symMap = { USD: "$", EUR: "أ¢â€ڑآ¬", GBP: "آآ£", SAR: "ر.س", AED: "د.إ", KWD: "د.ظئ’", QAR: "ر.ق", BHD: "د.ب", OMR: "ر.ع", JOD: "د.ا" };
...
function ensureOrdersModernStyles(){
  ...
  style.textContent = [
    ".orders-modern-page{--orders-bg:var(--bg-app,var(--site-accent-runtime-surface,#f1f1f4));--orders-soft:#e9ebf3;--orders-line:#cfd3e4;--orders-text:#18233f;--orders-muted:#777d8e;--orders-primary:#18233f;--orders-green:#53b69c;--orders-red:#d60945;--orders-gold:#c58900;direction:rtl;color:var(--orders-text);}",
    "html[data-theme='dark'] .orders-modern-page,body.dark-mode .orders-modern-page{--orders-bg:#07090f;--orders-soft:#111522;--orders-line:#2b3150;--orders-text:#f4f6ff;--orders-muted:#a2a8bd;--orders-primary:#dfe6ff;--orders-green:#72d7b2;--orders-red:#ff4f7e;--orders-gold:#f5cf59;}",
    ".orders-modern-shell{width:min(100%,980px);margin:0 auto;padding:22px clamp(10px,3vw,28px) 76px;background:transparent!important;border:0!important;box-shadow:none!important;outline:0!important;color:var(--orders-text)!important;}",
    ".orders-modern-topbar,.orders-subpage-topbar{display:grid;grid-template-columns:minmax(110px,1fr) auto minmax(110px,1fr);align-items:center;gap:12px;margin:0 0 28px;direction:ltr;}",
    ".orders-modern-topbar h2,.orders-subpage-topbar h2{margin:0!important;color:var(--orders-text)!important;font-size:clamp(24px,4.3vw,32px);font-weight:900;line-height:1.15;text-align:center;white-space:nowrap;}",
    ".orders-modern-tools{display:flex;justify-content:flex-start;gap:14px;direction:ltr;}",
    ".orders-modern-icon,.orders-modern-back,.orders-subpage-back{width:38px;height:38px;display:inline-grid;place-items:center;border:0;border-radius:50%;background:transparent;color:var(--orders-primary);font-size:24px;cursor:pointer;padding:0;}",
    ".orders-modern-back,.orders-subpage-back{justify-self:end;}",
    ".orders-modern-hidden-control{position:absolute!important;width:1px!important;height:1px!important;overflow:hidden!important;clip:rect(0 0 0 0)!important;white-space:nowrap!important;opacity:0!important;pointer-events:none!important;}",
    ".orders-modern-list{display:block!important;margin:0!important;padding:0!important;border:0!important;background:transparent!important;box-shadow:none!important;outline:0!important;}",
    "#ordersContainer .orders-date-group{margin:28px 0 14px;text-align:right;color:var(--orders-muted)!important;font-size:clamp(18px,3.4vw,24px);font-weight:800;line-height:1.2;}",
    "#ordersContainer .order-card{border:0!important;border-radius:0!important;background:transparent!important;box-shadow:none!important;padding:0!important;margin:0 0 26px!important;color:var(--orders-text)!important;overflow:visible!important;}",
    "#ordersContainer .order-header{display:grid;grid-template-columns:minmax(104px,146px) minmax(0,1fr) 44px;align-items:center;gap:12px;direction:ltr;cursor:pointer;padding:0!important;background:transparent!important;border:0!important;outline:0!important;-webkit-tap-highlight-color:transparent!important;user-select:none;}",
    "#ordersContainer .order-status{grid-column:1;grid-row:1;display:flex!important;flex-direction:column!important;align-items:flex-start!important;justify-content:center!important;gap:3px!important;text-align:left;direction:ltr;font-size:clamp(14px,2.35vw,17px);line-height:1.2;font-weight:900;color:var(--orders-muted)!important;background:transparent!important;border:0!important;padding:0!important;min-width:0;white-space:normal!important;}",
    "#ordersContainer .order-status.is-approved{color:var(--orders-green)!important;font-weight:900!important;}#ordersContainer .order-status.is-rejected{color:var(--orders-red)!important;}#ordersContainer .order-status.is-pending{color:var(--orders-gold)!important;}",
    "#ordersContainer .order-status .order-status-text{display:block!important;order:1;line-height:1.2;white-space:nowrap;}",
    "#ordersContainer .order-status .order-amount-mini{display:block!important;order:2;margin-top:0!important;font-size:clamp(11px,1.7vw,12.5px);font-weight:700;color:var(--orders-muted)!important;direction:ltr;text-align:left;letter-spacing:.2px;white-space:nowrap;}",
    "#ordersContainer .order-header-text{grid-column:2;grid-row:1;text-align:right;direction:rtl;min-width:0;}",
    "#ordersContainer .order-code-line{font-size:clamp(15px,2.45vw,18px);line-height:1.45;color:var(--orders-muted)!important;font-weight:850;white-space:normal;overflow:visible;text-overflow:clip;overflow-wrap:anywhere;direction:ltr;unicode-bidi:plaintext;text-align:right;}",
    "#ordersContainer .order-code-line strong{color:var(--orders-muted)!important;}#ordersContainer .order-code{color:var(--orders-muted)!important;}",
    "#ordersContainer .order-meta-line{font-size:clamp(17.5px,3vw,23px);line-height:1.3;color:var(--orders-text)!important;font-weight:900;white-space:normal;overflow-wrap:anywhere;}",
    "#ordersContainer .order-meta-line strong{color:var(--orders-text)!important;font-weight:900!important;}",
    "#ordersContainer .order-header-text{min-width:0!important;max-width:100%!important;overflow:hidden!important;}",
    "#ordersContainer .order-code-line,#ordersContainer .order-meta-line{display:block!important;min-width:0!important;max-width:100%!important;overflow-x:auto!important;overflow-y:hidden!important;white-space:nowrap!important;text-overflow:clip!important;overflow-wrap:normal!important;word-break:normal!important;scrollbar-width:none!important;-webkit-overflow-scrolling:touch!important;overscroll-behavior-x:contain!important;}",
    "#ordersContainer .order-code-line *,#ordersContainer .order-meta-line *{white-space:nowrap!important;overflow-wrap:normal!important;word-break:normal!important;}",
    "#ordersContainer .order-code-line::-webkit-scrollbar,#ordersContainer .order-meta-line::-webkit-scrollbar{display:none!important;}",
    "#ordersContainer .order-header>.fas{grid-column:3;grid-row:1;width:44px;height:44px;border-radius:50%;display:grid;place-items:center;background:#e4e6ee;color:var(--orders-primary);font-size:16px;justify-self:end;transition:transform .18s ease;outline:0!important;box-shadow:none!important;-webkit-tap-highlight-color:transparent!important;}",
    "html[data-theme='dark'] #ordersContainer .order-header>.fas,body.dark-mode #ordersContainer .order-header>.fas{background:#202536;border:0!important;box-shadow:none!important;outline:0!important;}",
    "#ordersContainer .order-card.open .order-header>.fas{transform:rotate(180deg);}",
    "#ordersContainer .order-details{margin:18px 0 0!important;padding:0!important;border:0!important;border-radius:0!important;background:transparent!important;color:var(--orders-text)!important;text-align:right;direction:rtl;box-shadow:none!important;}",
    "#ordersContainer .order-details p{margin:0!important;padding:9px 0!important;border-bottom:1px solid rgba(var(--site-accent-rgb,125,140,175),.24)!important;color:var(--orders-text)!important;font-size:clamp(17px,2.65vw,21px);line-height:1.75;}#ordersContainer .order-details p:first-child{padding-top:0!important;}#ordersContainer .order-details p:last-child{padding-bottom:0!important;border-bottom:0!important;}#ordersContainer .order-details p.order-reply-label{margin:0 0 8px!important;padding:0!important;border-bottom:0!important;}#ordersContainer .order-details strong{color:var(--orders-muted)!important;font-weight:850;}",
    "#ordersContainer .order-category-line{margin:0!important;}#ordersContainer .order-details .offers-list{list-style:none!important;margin:8px 0 0!important;padding:0!important;}#ordersContainer .order-details .offers-list li{list-style:none!important;margin:0!important;}#ordersContainer .order-offers-line{margin:10px 0!important;}#ordersContainer .order-offers-line,#ordersContainer .order-offers-line *{font-size:clamp(18px,2.9vw,23px)!important;line-height:1.7!important;}#ordersContainer .order-offers-value{margin-top:4px!important;font-weight:900!important;color:var(--orders-text)!important;}",
    "#ordersContainer .order-reply-field{margin-top:12px!important;margin-bottom:2px!important;}#ordersContainer .order-reply-label{font-size:clamp(14px,2.05vw,16px)!important;color:var(--orders-muted)!important;font-weight:850!important;}#ordersContainer .order-reply-value{margin-top:6px!important;padding:11px 13px!important;border:1px solid var(--orders-line)!important;border-radius:11px!important;background:rgba(255,255,255,.28)!important;color:var(--orders-text)!important;font-size:clamp(14px,2.1vw,17px)!important;line-height:1.5!important;font-weight:750!important;white-space:pre-wrap!important;overflow-wrap:anywhere!important;outline:0!important;box-shadow:none!important;-webkit-tap-highlight-color:transparent!important;}#ordersContainer .order-reply-value:focus,#ordersContainer .order-reply-value:active{outline:0!important;box-shadow:none!important;}#ordersContainer .order-reply-value:focus-visible{box-shadow:0 0 0 2px rgba(125,140,175,.22)!important;}html[data-theme='dark'] #ordersContainer .order-reply-value,body.dark-mode #ordersContainer .order-reply-value{background:#0d111b!important;}",
    ".orders-modern-empty{min-height:260px;margin:36px auto 0;display:grid;place-items:center;align-content:center;gap:12px;border:0!important;background:transparent!important;box-shadow:none!important;text-align:center;color:var(--orders-text)!important;}",
    ".orders-modern-empty i{font-size:46px;color:var(--orders-muted);}.orders-modern-empty .caption{font-size:16px;font-weight:900;color:var(--orders-text)!important;line-height:1.6;}",
    "#ordersPagination{border:0!important;background:transparent!important;box-shadow:none!important;margin:24px 0 0!important;}",
    ".orders-subpage-active .orders-modern-topbar{display:none!important;}.orders-filter-active #ordersList,.orders-filter-active #ordersPagination{display:none!important;}",
    ".orders-modern-subpage{display:none;color:var(--orders-text)!important;}.orders-modern-subpage.show{display:block;}",
    ".orders-subpage-searchbox{height:58px;border:1px solid var(--orders-line);border-radius:12px;background:var(--orders-soft);display:grid;grid-template-columns:52px minmax(0,1fr);align-items:center;direction:ltr;margin-bottom:26px;}",
    ".orders-subpage-searchbox i{font-size:24px;color:var(--orders-primary);text-align:center;}.orders-subpage-searchbox input{width:100%;height:100%;border:0!important;background:transparent!important;color:var(--orders-text)!important;font-size:19px;outline:none;text-align:right;direction:rtl;padding:0 16px;}",
    ".orders-subpage-section-label{margin:0 0 20px;text-align:right;color:var(--orders-muted);font-size:19px;font-weight:800;}",
    ".orders-filter-section{margin:0 0 30px;}.orders-filter-section h3{margin:0 0 14px;color:var(--orders-text)!important;font-size:22px;line-height:1.2;font-weight:900;text-align:right;}",
    ".orders-filter-strip{display:flex;gap:8px;overflow-x:auto;scrollbar-width:none;padding-bottom:2px;}.orders-filter-strip::-webkit-scrollbar{display:none;}",
    ".orders-filter-strip button{flex:0 0 auto;min-width:72px;min-height:40px;border:1px solid var(--orders-line);border-radius:12px;background:var(--orders-soft);color:var(--orders-muted);font-size:14px;font-weight:900;padding:0 14px;cursor:pointer;}",
    ".orders-filter-strip button.active{background:var(--orders-primary);border-color:var(--orders-primary);color:var(--orders-bg);}",
    ".orders-filter-actions{display:grid;gap:10px;margin-top:34px;}.orders-filter-actions button{min-height:52px;border-radius:14px;font-size:17px;font-weight:900;cursor:pointer;}",
    ".orders-filter-actions .primary{border:0;background:var(--orders-primary);color:var(--orders-bg);}.orders-filter-actions .ghost{border:1px solid var(--orders-muted);background:transparent;color:var(--orders-muted);}",
    "@media(max-width:520px){.orders-modern-shell{padding-inline:6px;}.orders-modern-topbar,.orders-subpage-topbar{grid-template-columns:64px minmax(0,1fr) 64px;margin-bottom:24px;}.orders-modern-icon,.orders-modern-back,.orders-subpage-back{width:34px;height:34px;font-size:21px;}#ordersContainer .order-header{grid-template-columns:minmax(58px,72px) minmax(0,1fr) 42px;gap:8px;}#ordersContainer .order-header>.fas{width:42px;height:42px;font-size:15px;}#ordersContainer .order-status{font-size:14px;}#ordersContainer .order-code-line{font-size:14.5px;line-height:1.5;}#ordersContainer .order-meta-line{font-size:18px;line-height:1.38;}#ordersContainer .order-details p{font-size:16.5px;line-height:1.78;}#ordersContainer .order-offers-line,#ordersContainer .order-offers-line *{font-size:17px!important;}#ordersContainer .order-reply-value{font-size:14px!important;line-height:1.48!important;}.orders-filter-strip button{min-width:66px;min-height:38px;font-size:13px;padding-inline:12px;}}"
  ].join("");
...
function ensureOrdersFilterPage(){
  ...
  page.innerHTML = [
    ...
    '  <h3>اختر التاريخ</h3>',
    '  <div class="orders-filter-strip" data-orders-date-strip="1">',
    '    <button type="button" data-orders-date="custom">مخصص</button>',
    '    <button type="button" data-orders-date="all">الكل</button>',
    '    <button type="button" data-orders-date="today">اليوم</button>',
    '    <button type="button" data-orders-date="yesterday">أمس</button>',
    '  </div>',
    '</div>',
    '<div class="orders-filter-section">',
    '  <h3>اختر الحالة</h3>',
    '  <div class="orders-filter-strip" data-orders-status-strip="1">',
    '    <button type="button" data-orders-filter="all">الكل</button>',
    '    <button type="button" data-orders-filter="pending">قيد الانتظار</button>',
    '    <button type="button" data-orders-filter="approved">مقبول</button>',
    '    <button type="button" data-orders-filter="rejected">مرفوض</button>',
    '  </div>',
    '</div>',
    '<div class="orders-filter-actions">',
    '  <button type="button" class="primary" data-orders-filter-apply="1">تصفية</button>',
    '  <button type="button" class="ghost" data-orders-filter-reset="1">مسح</button>',
    '</div>'
  ].join("");
  shell.insertBefore(page, document.getElementById("ordersList"));
  return page;
...
function syncOrdersModernFilterUI(){
  ...
  page.querySelectorAll("[data-orders-filter]").forEach(btn => {
    btn.classList.toggle("active", String(btn.getAttribute("data-orders-filter") || "all") === ORDERS_FILTER);
  });
  const dateMode = SELECTED_DATE_STR === "__all__"
    ? "all"
    : (DATE_MODE === "range" || SELECTED_DATE_MANUAL ? "custom" : (SELECTED_DATE_STR === getOrdersYesterdayStr() ? "yesterday" : "today"));
  page.querySelectorAll("[data-orders-date]").forEach(btn => {
    btn.classList.toggle("active", String(btn.getAttribute("data-orders-date") || "") === dateMode);
  });
}
...
function closeOrdersSubpages(){
  ...
  root.classList.remove("orders-subpage-active", "orders-search-active", "orders-filter-active");
...
function openOrdersSearchPage(){
  const root = document.getElementById("ordersContainer");
  const page = ensureOrdersSearchPage();
  if (!root || !page) return;
  const input = page.querySelector("#ordersSearchPageInput");
  if (input) input.value = SEARCH_QUERY || "";
  ...
  if (filterPage) filterPage.classList.remove("show");
  try { setTimeout(() => { if (input) input.focus(); }, 30); } catch(_){}
}

function openOrdersFilterPage(){
  const root = document.getElementById("ordersContainer");
  const page = ensureOrdersFilterPage();
  if (!root || !page) return;
...
function bindOrdersModernControls(){
  ...
  root.addEventListener("click", function(e){
    ...
    if (target && target.closest && target.closest("#ordersModernBack")) {
      e.preventDefault();
      if (root.classList.contains("orders-subpage-active")) {
        closeOrdersSubpages();
    ...
    const statusBtn = target && target.closest ? target.closest("[data-orders-filter]") : null;
    if (statusBtn && root.contains(statusBtn)) {
      e.preventDefault();
      ORDERS_FILTER = String(statusBtn.getAttribute("data-orders-filter") || "all");
      syncToolbarUI();
      syncOrdersModernFilterUI();
      return;
    }
    const dateBtn = target && target.closest ? target.closest("[data-orders-date]") : null;
    if (dateBtn && root.contains(dateBtn)) {
      ...
      if (mode === "all") {
        ...
        if (!SELECTED_DATE_STR || SELECTED_DATE_STR === "__all__") SELECTED_DATE_STR = getTodayStr();
        SELECTED_DATE_MANUAL = true;
        syncOrdersModernFilterUI();
        openCalendar();
        return;
      } else {
    ...
    if (target && target.closest && target.closest("[data-orders-filter-apply]")) {
      e.preventDefault();
      closeOrdersSubpages();
      recomputeAndRender();
      return;
    }
    if (target && target.closest && target.closest("[data-orders-filter-reset]")) {
      e.preventDefault();
      resetOrdersModernFilters();
      return;
    }
  });
  root.addEventListener("input", function(e){
    ...
    const hiddenSearch = document.getElementById("ordersSearch");
    if (hiddenSearch) hiddenSearch.value = SEARCH_QUERY;
    recomputeAndRender();
  });
}
// تحويل ناعم داخل الـSPA عند غياب الجلسة: لا alert ولا إعادة تحميل.
function routeOrdersToLogin(){
  try {
    if (!isOrdersViewActive()) return;
    if (typeof window.navigateHomeHash === 'function') {
      window.navigateHomeHash('#/login', 'login');
      return;
    }
  } catch (_) {}
  try { location.hash = '#/login'; } catch (_) {}
}
// تحقق دخول مرة واحدة عند فتح صفحة الطلبات.
async function bindOrdersAuthListener(){
  ...
  try {
    ...
    if (!user) {
      ...
      // لا إعادة تحميل كاملة: كانت location.href="index.html" تُسقِط الـhash
      // وتُرجع الزائر للرئيسية بعد أن فتحت الطلبات فعلاً (عطل الشريط السفلي).
      routeOrdersToLogin();
      return;
    }
...
window.__initOrdersPage = function(){
  ...
  try { ensureOrdersModernStyles(); bindOrdersModernControls(); } catch(_){}
...
// أداة: حساب إن كان الطلب حديثًا (أ¢â€°آ¤ N أيام)
...
/**
 * يجلب الطلبات الحديثة (أ¢â€°آ¤ 7 أيام) من المستند المجمّع لتحديث الحقول (خصوصًا status).
...
async function refreshRecentStatuses(uid) {
  ...
  const recentCodes = codes.filter(code => {
    ...
    // نحدّث فقط إذا كان الطلب حديثًا (أ¢â€°آ¤ 7 أيام)
...
function drawOrdersPage() {
  ...
  try { ordersList.querySelectorAll(".order-card.loading").forEach(n => n.remove()); } catch (_) {}
  try { ordersList.querySelectorAll(".orders-date-group").forEach(n => n.remove()); } catch (_) {}
  ...
  if (total === 0) {
    ...
    if (ORDERS_FILTER === 'approved') {
      ...
      message = ordersT("orders.empty.pending", "لا توجد طلبات قيد الانتظار");
    }
    // عبارة بحسب وضع التاريخ
    if (SELECTED_DATE_STR !== "__all__") {
  ...
  slice.forEach(order => {
    ...
    const labelReply = ordersT("orders.label.reply", "الرد");
    const actionCopyReply = ordersT("orders.action.copyReply", "نسخ الرد");
    ...
    // عنوان رأس الكرت: اسم المنتج، بينما يظهر القسم داخل التفاصيل.
    ...
    const categoryDisplayValue = (() => {
      ...
      if (gameValue && gameValue !== "-") return stripOrderProductsPrefix(gameValue) || gameValue;
      return "";
    })();
    ...
    const safeHeaderDisplay = escapeHtml(productDisplayValue || "-");
    ...
    const labelOrderCode = ordersT("orders.label.code", "كود الطلب");
    const labelBuy = ordersT("orders.label.buy", "شراء");
    const labelCategory = ordersT("orders.label.category", "القسم");
    const labelRefund = ordersT("orders.label.refund", "حالة الاسترداد");
    const labelTotal = ordersT("orders.label.total", "المجموع");
    const labelDate = ordersT("orders.label.sendDate", "تاريخ الإرسال");
    const labelExecutionDuration = ordersT("orders.label.executionDuration", "مدة التنفيذ");
    const labelProof = ordersT("orders.label.proof", "إثبات التحويل");
    const labelPlayerId = ordersT("orders.label.playerId", "ايدي اللاعب");
    const textShowProof = ordersT("orders.action.showProof", "عرض الصورة");
    const textHideProof = ordersT("orders.action.hideProof", "إخفاء الصورة");
    const categoryLineHtml = categoryDisplayValue
    ...
    const refundAmountDisplay = (refundAmountStr || refundAmountHasValue)
      ...
      : "";
    const executionDurationText = formatOrderExecutionDuration(order);
    const executionDurationHtml = executionDurationText
...
function syncToolbarUI(){
  try{
    ...
    try { syncOrdersModernFilterUI(); } catch {}
    return true;
  }catch{ return false; }
}
...
function attachProofButtons() {
  document.querySelectorAll('.btn-show-proof').forEach(btn => {
    btn.onclick = () => {
      ...
      const showText = String(btn.dataset.showText || ordersT("orders.action.showProof", "عرض الصورة"));
      const hideText = String(btn.dataset.hideText || ordersT("orders.action.hideProof", "إخفاء الصورة"));
  ...
  document.querySelectorAll(".btn-refresh-order").forEach((btn) => {
    btn.onclick = async (e) => {
      ...
      if (!orderUuid && !orderId) {
        alert(ordersT("orders.alert.noProviderTracking", "لا يوجد رقم تتبع للمزود لهذا الطلب."));
        return;
      }
      ...
      try {
        btn.disabled = true;
        btn.textContent = ordersT("orders.action.updating", "جارٍ التحديث...");

        const headers = {};
        const session = readOrdersSessionContext(uid);
        const sessionKey = String(session.sessionKey || "").trim();
        if (!sessionKey) throw new Error("رمز الجلسة غير متوفر. سجّل الدخول مرة أخرى ثم أعد المحاولة.");
        if (uid) headers["X-UserUid"] = uid;
        // النقل الموحّد: الجلسة تركب كوكي الجلسة وحده — فحص الوجود أعلاه
        // يبقى (تجربة مستخدم أوضح من 401)، لكن المفتاح لا يُرسل.
        try {
        ...
        const url = buildProviderCheckUrl({ orderUuid, orderId, useruid: uid, sessionKey });
        const res = await fetch(url, { method: "GET", headers });
        const data = await res.json().catch(() => ({}));
        if (!res.ok || data?.success === false || data?.ok === false) {
          throw new Error(data?.error || data?.message || ordersT("orders.error.refreshFailed", "فشل تحديث الرد"));
        }
        ...
        alert(err?.message || ordersT("orders.error.refreshFailed", "فشل تحديث الرد"));
      } finally {
        btn.disabled = false;
        btn.textContent = prevText || ordersT("orders.action.refreshReply", "تحديث الرد");
      }
    };
  });
}
  ...
  document.querySelectorAll('.btn-copy-code').forEach(btn => {
    btn.onclick = () => {
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        ...
        }).catch(() => alert(ordersT("orders.error.copyFailed", "تعذر نسخ الكود، انسخه يدويًا.")));
      } else {
        alert(ordersT("orders.error.copyManualPrefix", "انسخ الكود يدويًا: ") + code);
      }
    };
  });
}
...
function attachCopyReplyButtons() {
  ...
  const textCopyFailed = ordersT("orders.error.copyReplyFailed", "تعذر نسخ الرد، انسخه يدويًا.");
  const textCopyManualPrefix = ordersT("orders.error.copyReplyManualPrefix", "انسخ الرد يدويًا: ");
...
function renderDetailsTable(pub, priv, detailsBox) {
  ...
  rows += `<tr><td colspan="2" style="background:#eee;padding:10px;font-weight:bold;">ًں“‚ تفاصيل الطلب</td></tr>`;
