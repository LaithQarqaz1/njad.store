/* Account/API/transfer route builders — extracted from the router IIFE in
   ...
   window. The router keeps lazy thunks + the renderFromTemplate fallback. */
      ...
      var transferRoute = (function(){
        ...
        // Per-store key from site_settings.js (GENERATED from store.identity.json)
        // — never a literal, which would belong to one domain only.
        var DEFAULT_TURNSTILE_SITEKEY = (function(){
          try {
            return String((window.__getSiteSetting ? window.__getSiteSetting("security.turnstileSiteKey", "") : "") || "").trim();
          } catch (_) {
            return "";
          }
        })();
        ...
        function bindAuth(){
          ...
          state.unsubAuth = state.auth.onAuthStateChanged(function(user){
            state.currentUser = user || null;
            if (!user){
              // الدخول الموحّد (كوكي الجلسة / sessionKey) لا يضمن مستخدم Firebase
              // حيّاً في المتصفح — كان هذا الفرع يعامل مستخدم الجلسة كزائر
              // («سجّل الدخول» + «تعذر تحميل معلومات المستخدم») رغم أن جلب
              // الملف يمرّ بالخادم حصراً عبر sessionKey. نفس سلّم المحفظة.
              var sessionUser = resolveTransferSessionUser();
              if (sessionUser){
        ...
        // مستخدم جلسة موحّدة {uid} من مخازن الدخول عندما لا يوجد مستخدم
        // Firebase حيّ — يكفي loadProfile لأن الجلب خادمي بمفتاح الجلسة.
        function resolveTransferSessionUser(){
          var candidates = [];
          function push(raw){
            try {
              var item = typeof raw === "string" ? JSON.parse(raw || "null") : raw;
              if (item && typeof item === "object") candidates.push(item);
            } catch(_){}
          }
          try { push(localStorage.getItem("sessionKeyInfo")); } catch(_){}
          try { push(localStorage.getItem("postLoginPayload")); } catch(_){}
          try {
            var bundleKey = String(window.__AUTH_SESSION_BUNDLE_STORAGE_KEY__ || "auth:session:bundle:v1");
            var bundle = JSON.parse(localStorage.getItem(bundleKey) || "null");
            if (bundle && typeof bundle === "object"){
              push(bundle.sessionKeyInfo);
              push(bundle.postLoginPayload);
            }
          } catch(_){}
          for (var i = 0; i < candidates.length; i += 1){
            var item = candidates[i] || {};
            var uid = String(item.uid || item.useruid || item.userUid || "").trim();
            var sessionKey = String(item.sessionKey || item.session_key || "").trim();
            if (uid && sessionKey){
              var storedToken = String(item.idToken || item.id_token || item.firebaseIdToken || item.token || "").trim();
              // شكل متوافق مع مستخدم Firebase: كثير من مسارات التحويل تنادي
              // user.getIdToken(...)؛ الجلسة الموحّدة قد لا تحمل توكناً حياً،
              // والمصادقة الفعلية بمفتاح الجلسة، فنعيد التوكن المخزن أو "".
              return {
                uid: uid,
                viaSession: true,
                getIdToken: function(){ return Promise.resolve(storedToken); }
              };
            }
          }
          return null;
        ...
        function loadProfile(user, options){
          // كان الشرط يتطلب state.db (عميل Firestore) — بقايا عصرٍ كان الملف
          // يُقرأ منه مباشرة؛ الجلب اليوم خادمي حصراً (__fetchAccountInfoFromServer
          // بمفتاح الجلسة) فاشتراط db كان يفشل مستخدمي الجلسة بلا سبب.
          if (!user){
        ...
        // «تحويل الرصيد» master switch (siteState.transfers.enabled). Mirrors the
        // comments section: when an admin turns it off the route renders a
        // "section stopped" notice instead of the form, so even a direct
        // #/transfer visit cannot start a transfer (the backend rejects it too).
        // Defaults to ENABLED (fail-open) while siteState is still resolving.
        function transfersSectionEnabled(){
          try {
            if (typeof window.__isSiteTransfersEnabled === 'function') {
              return window.__isSiteTransfersEnabled() !== false;
            }
          } catch (_) {}
          return true;
        }
        function buildTransferDisabledFragment(){
          var frag = document.createDocumentFragment();
          var wrap = document.createElement('div');
          wrap.className = 'transfer-page';
          wrap.innerHTML = [
            '<main>',
            '<h2><span><i class="fa-solid fa-right-left"></i> تحويل الرصيد</span></h2>',
            '<div style="padding:30px 18px;text-align:center;color:var(--text,#1d2742);border:1px solid rgba(var(--site-accent-rgb,148,163,184),.36);border-radius:18px;font-weight:800;line-height:1.9;">',
            '<i class="fa-solid fa-lock" style="font-size:30px;display:block;margin:0 0 12px;color:var(--site-accent-runtime,#5c5ebf);"></i>',
            'قسم تحويل الرصيد متوقف حالياً.',
            '</div>',
            '</main>'
          ].join('');
        ...
        function build(){
          if (!transfersSectionEnabled()) return buildTransferDisabledFragment();
          var frag = document.createDocumentFragment();
          frag.appendChild(ensureRoot());
          bindAuth();
          return frag;
        ...
        function onShow(){
          if (!transfersSectionEnabled()) return Promise.resolve(true);
          ensureRoot();
          var authReady = bindAuth();
          var tasks = [];
          return Promise.resolve(authReady).then(function(){
            var effectiveUser = state.currentUser || resolveTransferSessionUser();
            if (effectiveUser && !state.currentUser) state.currentUser = effectiveUser;
            if (effectiveUser && !state.profile){
              tasks.push(loadProfile(effectiveUser));
            }
            if (effectiveUser) tasks.push(syncTotpStatus(false));
            updateHelperText();
