(function(){
  ...
  const TOTP_EMAIL_RESEND_COOLDOWN_SECONDS = 5 * 60;
  ...
  const savePostLoginPayload = (payload = {}) => {
    ...
    try {
      // window.name mirror carries NON-SECRET display fields only.
  ...
  function resolveManualAuthErrorCode(payload, fallbackCode) {
    ...
    if (direct.includes("LOGIN_RATE_LIMITED")) return "auth/login-rate-limited";
    if (direct.includes("TOTP_ATTEMPTS_LOCKED")) return "totp_attempts_locked";
    ...
    if (has("LOGIN_RATE_LIMITED")) return "auth/login-rate-limited";
    ...
    if (has("TOTP_ATTEMPTS_LOCKED")) return "totp_attempts_locked";
  ...
  function getOtpCurrentUid() {
    try {
      if (typeof getCurrentAuthUid === "function") {
        const uid = String(getCurrentAuthUid() || "").trim();
        if (uid) return uid;
      }
    } catch (_) {}
    try {
  ...
  (function installSiteOtpLimits(){
    if (window.__siteOtpLimits) return;
    const WINDOW_MS = 5 * 60 * 1000;
    function storageKey(scope) {
      return "siteOtpLimit:" + encodeURIComponent(String(scope || "default")).slice(0, 180);
    }
    function read(scope) {
      try {
    ...
    function write(scope, state) {
      try { localStorage.setItem(storageKey(scope), JSON.stringify(state || {})); } catch (_) {}
    }
    function cleanAttempts(list, now) {
      return (Array.isArray(list) ? list : [])
        .map((item) => Number(item) || 0)
        .filter((ts) => ts > 0 && (now - ts) < WINDOW_MS);
    }
    function remainingFromStart(ts, now) {
      const left = WINDOW_MS - Math.max(0, now - (Number(ts) || 0));
    ...
    window.__siteOtpLimits = {
      getSendRemaining(scope) {
        const state = read(scope);
        return remainingFromStart(state.sentAt, Date.now());
      },
      markSent(scope) {
        const state = read(scope);
      ...
      getAttemptRemaining(scope, maxAttempts) {
        const now = Date.now();
        const state = read(scope);
        const attempts = cleanAttempts(state.attempts, now);
        state.attempts = attempts;
        write(scope, state);
        const max = Math.max(1, Number(maxAttempts) || 1);
        return attempts.length >= max ? remainingFromStart(attempts[0], now) : 0;
      },
      markAttempt(scope) {
        const now = Date.now();
        const state = read(scope);
  ...
  function getTotpCodeDeliveryButtonHtml(method, requestedBefore = false) {
    return requestedBefore
      ? 'إعادة إرسال رمز عبر تيليغرام'
      : 'إرسال رمز عبر تيليغرام';
  }
    ...
    if (totpLoginEmailBtn) {
      ...
      if (!canRequestEmail) {
        ...
        if (pendingTotpEmailBusy) {
          ...
          totpLoginEmailBtn.textContent = 'جاري الإرسال...';
        } else {
          const remaining = getTotpModalCooldownRemaining("email");
          if (remaining > 0) {
            totpLoginEmailBtn.disabled = true;
            totpLoginEmailBtn.textContent = 'إعادة الإرسال بعد ' + remaining + " ث";
          } else {
  ...
  function formatRetryAfterSeconds(seconds) {
    const sec = Math.max(0, Math.ceil(Number(seconds) || 0));
    if (window.__siteOtpLimits && typeof window.__siteOtpLimits.format === "function") {
      return window.__siteOtpLimits.format(sec);
    }
    const m = Math.floor(sec / 60);
    const s = sec % 60;
    return m > 0 ? (m + ":" + String(s).padStart(2, "0") + " دقيقة") : (s + " ثانية");
  }

  function appendRetryAfterMessage(message, input) {
    const retryAfter = getRetryAfterSecondsFromError(input);
    const base = String(message || "").trim();
    if (retryAfter <= 0 || /(?:بعد|خلال)\s+\d/.test(base)) return base;
    return base + " المحاولة التالية بعد " + formatRetryAfterSeconds(retryAfter) + ".";
  }
  ...
  async function requestTotpEmailCodeFromModal() {
    ...
    const otpScope = `login-send:${requestMethod || "telegram"}:${typeof getOtpCurrentUid === "function" ? getOtpCurrentUid() : ""}`;
    const localRemaining = (requestMethod === "telegram" && window.__siteOtpLimits)
      ? window.__siteOtpLimits.getSendRemaining(otpScope)
      : 0;
    const remaining = Math.max(getTotpModalCooldownRemaining("email"), localRemaining);
    if (remaining > 0) {
    ...
    try {
      ...
      if (requestMethod === "telegram" && window.__siteOtpLimits) {
        window.__siteOtpLimits.markSent(otpScope);
      }
      const safeCooldownSeconds = Number.isFinite(cooldownSeconds) && cooldownSeconds > 0 ? cooldownSeconds : 300;
      startTotpModalCooldown("email", Math.max(300, safeCooldownSeconds));
  ...
  async function requestTotpLostRecoveryFromModal() {
    ...
    const totpLoginInput = byId("totpLoginInput");
    const totpLoginConfirm = byId("totpLoginConfirm");
    const otpScope = `login-send:lost:${typeof getOtpCurrentUid === "function" ? getOtpCurrentUid() : ""}`;
    const localRemaining = window.__siteOtpLimits ? window.__siteOtpLimits.getSendRemaining(otpScope) : 0;
    const remaining = Math.max(getTotpModalCooldownRemaining("lost"), localRemaining);
    if (remaining > 0) {
    ...
    try {
      ...
      if (window.__siteOtpLimits) window.__siteOtpLimits.markSent(otpScope);
      const safeCooldownSeconds = Number.isFinite(cooldownSeconds) && cooldownSeconds > 0 ? cooldownSeconds : 300;
      startTotpModalCooldown("lost", Math.max(300, safeCooldownSeconds));
  ...
  function openTotpModal(opts){
    ...
    const totpLoginTitle = totpLoginModal.querySelector(".totp-modal h3");
    const totpLoginSubtitle = byId("totpLoginSubtitle");
    const totpLoginError = byId("totpLoginError");
    const totpLoginInput = byId("totpLoginInput");
    ...
    if (totpLoginTitle) totpLoginTitle.textContent = "أدخل رمز التحقق";
    if (totpLoginSubtitle) {
  ...
  async function confirmTotpModal() {
    ...
    const challengeMethod = pendingTotpLostMode
      ? "telegram"
      : (normalizeTotpChallengeMethod(totpModalMethod) || "app");
    const maxAttempts = challengeMethod === "telegram" ? 3 : 5;
    const attemptScope = `login-attempt:${challengeMethod}:${pendingTotpLostMode ? "lost" : "auth"}:${typeof getOtpCurrentUid === "function" ? getOtpCurrentUid() : ""}`;
    const blockedFor = window.__siteOtpLimits
      ? window.__siteOtpLimits.getAttemptRemaining(attemptScope, maxAttempts)
      : 0;
    if (blockedFor > 0) {
      if (totpLoginError) {
        totpLoginError.style.color = "var(--danger, #ef4444)";
        totpLoginError.textContent = "انتهت محاولات إدخال الرمز. حاول بعد " + window.__siteOtpLimits.format(blockedFor) + ".";
      }
      return;
    }
  ...
  // This store's own hostname, for the messages that must tell the customer
  // which address to open the site from. Taken from site_settings.js (GENERATED
  // from store.identity.json) so it follows the store, never a literal domain.
  const SITE_HOST_LABEL = (function(){
    try {
      const origin = String((window.__getSiteSetting ? window.__getSiteSetting("seo.siteOrigin", "") : "") || "").trim();
      if (origin) return new URL(origin).hostname;
    } catch (_) {}
    try { return String(window.location.hostname || "").trim(); } catch (_) { return ""; }
  })();
  ...
  const firebaseErrorMessages = {
    ...
    "auth/missing-initial-state": "تعذر إكمال تسجيل Google بسبب فقدان حالة التحويل. افتح الموقع من " + (SITE_HOST_LABEL || "عنوانه الرسمي") + " واسمح بالتخزين المحلي ثم حاول مرة أخرى.",
    ...
    "auth/login-rate-limited": "تم إيقاف تسجيل الدخول لهذا البريد مؤقتًا.",
    ...
    "totp-attempts-locked": "انتهت محاولات إدخال رمز التحقق.",
  ...
  function translateFirebaseError(input) {
    const finish = (message) => appendRetryAfterMessage(message, input);
    const candidates = collectUiErrorMessageCandidates(input);
    const key = normalizeUiErrorCode(candidates[0] || input);
    if (!candidates.length && !key) return finish("حدث خطأ غير متوقڡ حاول مرة أخرى.");
    for (const candidate of candidates) {
      const candidateKey = normalizeUiErrorCode(candidate);
      if (!candidateKey) continue;
      if (candidateKey === "auth/unauthorized-domain") return finish(getUnauthorizedDomainMessage());
      if (firebaseErrorMessages[candidateKey]) return finish(firebaseErrorMessages[candidateKey]);
      if (candidateKey.includes("too-many-requests")) return finish(firebaseErrorMessages["auth/too-many-requests"]);
    }
    for (const candidate of candidates) {
      if (isHumanReadableErrorMessage(candidate)) return finish(candidate);
    }
    if (key === "auth/unauthorized-domain") return finish(getUnauthorizedDomainMessage());
    if (firebaseErrorMessages[key]) return finish(firebaseErrorMessages[key]);
    if (key.includes("too-many-requests")) return finish(firebaseErrorMessages["auth/too-many-requests"]);
    if (key.startsWith("auth/http-")) return finish(firebaseErrorMessages["network/fetch-failed"]);
    if (key.startsWith("network/")) return finish(firebaseErrorMessages["network/fetch-failed"]);
    if (key.startsWith("auth/")) return finish(firebaseErrorMessages["auth/unknown"]);
    return finish("حدث خطأ غير متوقڡ حاول مرة أخرى.");
  }
  ...
  let loginServerCooldownTimer = null;
  let loginServerCooldownUntilMs = 0;

  function isLoginServerCooldownActive() {
    return loginServerCooldownUntilMs > Date.now();
  }

  function startLoginServerCooldown(seconds, errorEl, buttonEl, message) {
    const sec = Math.max(1, Math.ceil(Number(seconds) || 0));
    loginServerCooldownUntilMs = Date.now() + sec * 1000;
    if (loginServerCooldownTimer) {
      try { clearInterval(loginServerCooldownTimer); } catch (_) {}
      loginServerCooldownTimer = null;
    }
    const baseMessage = String(message || "تم إيقاف تسجيل الدخول لهذا البريد مؤقتًا.")
      .replace(/\s*المحاولة التالية بعد\s+.+?\.$/, "")
      .trim();
    const tick = () => {
      const remaining = Math.max(0, Math.ceil((loginServerCooldownUntilMs - Date.now()) / 1000));
      if (remaining <= 0) {
        if (loginServerCooldownTimer) {
          try { clearInterval(loginServerCooldownTimer); } catch (_) {}
        }
        loginServerCooldownTimer = null;
        loginServerCooldownUntilMs = 0;
      ...
      if (errorEl) {
        ...
        errorEl.textContent = baseMessage + " المحاولة التالية بعد " + formatRetryAfterSeconds(remaining) + ".";
      }
    };
    tick();
    loginServerCooldownTimer = setInterval(tick, 1000);
  }
  ...
  async function performManualLogin(event) {
    ...
    if (isLoginServerCooldownActive()) return;
    ...
    while (true) {
          ...
          if ([
            ...
            const errorText = (code === "telegram_otp_invalid")
              ...
              : (code === "totp_code_invalid")
                ...
                : (code === "telegram_otp_expired"
                  ? "انتهت صلاحية رمز تيليغرام، اطلب رمزًا جديدًا."
                  : (["totp_attempts_locked", "telegram_otp_locked", "telegram_otp_rate_limited"].includes(code)
      ...
      const retryAfter = getRetryAfterSecondsFromError(err);
      const message = translateFirebaseError(err || code);
      if (retryAfter > 0) {
        startLoginServerCooldown(retryAfter, loginError, submitLogin, message);
      } else {
        loginError.textContent = message;
      }
    } finally {
      if (!isLoginServerCooldownActive()) {
        setButtonBusy(submitLogin, false);
      } else {
        submitLogin.setAttribute("aria-busy", "false");
        submitLogin.classList.remove("is-busy", "loading");
