...
// Warm the loader/preloader logo for LATER navigations. It is not shown on the
// home first paint, so warm it at LOW priority on idle — a high-priority preload
// here would compete with the home LCP image over a throttled connection.
(function(){
  try {
    ...
    const warm = function(){
      try {
        ...
        img.loading = 'lazy'; img.src = imgHref;
      } catch {}
    };
...
function shouldKeepPageLoaderVisible(){
  try {
    try {
      ...
      if (window.__LEVELS_INLINE_LOADING__ === true) return true;
      ...
      if (document.documentElement && document.documentElement.classList.contains('levels-loader-pending')) return true;
      if (document.body && document.body.classList.contains('levels-loader-pending')) return true;
    ...
    if (allowForceHide) {
      ...
      try {
        const levelsActive =
          ...
          (document.documentElement && document.documentElement.classList.contains('levels-loader-pending')) ||
          (document.body && document.body.classList.contains('levels-loader-pending'));
        if (levelsActive && !(opts && opts.ignoreLevelsLoading === true)) return;
      } catch {}
    }
...
(function setupSessionKeyGuard(){
  try {
    ...
    function requestCarriesSession(req){
      // كانت ترويسة X-SessionKey هي المؤشر؛ بعد النقل الموحّد الجلسة تركب
      // كوكي HttpOnly غير مرئي هنا — فاعتبر كل طلب لقاعدة الـAPI طلب جلسة
      // (الأكواد العامة لا تعيد أكواد session_* فلا أثر جانبي للاعتراض).
      try { if (req.headers.get(SESSION_HEADER)) return true; } catch (_) {}
      try {
        var base = (typeof window.__getSiteWorkerBaseDefault === 'function')
          ? String(window.__getSiteWorkerBaseDefault({ trailingSlash: false }) || '')
          : '';
        if (!base) return false;
        return new URL(req.url, location.href).origin === new URL(base, location.href).origin;
      } catch { return false; }
    }
...
(function setupCurrency(){
  try {
    ...
    try {
      ...
      window.convertToBaseCurrency = convertToJOD;
      window.getSiteCurrencyOptions = function(){
        try {
          const MAP = getRates();
          const codes = getSelectableCurrencyCodes(MAP);
          const keys = (codes && codes.length) ? codes : [getBaseCurrencyKey(MAP)].filter(Boolean);
          const seen = Object.create(null);
          const out = [];
          keys.forEach(function(key){
            const entry = resolveCurrencyEntry(key, MAP, { visibleOnly: false }) || {};
            const code = getCurrencyEntryCode(entry, key) || '';
            if (!code || seen[code]) return;
            seen[code] = true;
            out.push({
              code: code,
              key: key,
              symbol: getCurrencyEntrySymbol(entry, code) || code,
              name: getCurrencyEntryName(entry, code) || code
            });
          });
          if (!out.length) out.push({ code: STORE_BASE_CODE, key: STORE_BASE_CODE, symbol: '$', name: STORE_BASE_CODE });
          return out;
        } catch (_) {
          return [{ code: STORE_BASE_CODE, key: STORE_BASE_CODE, symbol: '$', name: STORE_BASE_CODE }];
        }
      };
...
// i18n fully removed — Arabic-only. The helpers below are NOT translation:
// they normalise Arabic-Indic digits to Latin and classify currency/text.
...
/* ---------------------------------------------------------------------------
   i18n REMOVED — the site is Arabic-only with no ar/en/fr translator.
   The runtime translation engine, the language dictionary, the language
   switcher and the alert/confirm translation patching were all deleted.
   What remains are inert shims so existing call sites keep working; the only
   real work they do is normalising Arabic-Indic digits to Latin (a display
   concern, not translation).
   --------------------------------------------------------------------------- */
function applyTranslations(root){ try { enforceLatinDigits(root || document); } catch (_) {} }
function translateKey(key, fallback){ return toLatinDigits(fallback != null ? fallback : (key || '')); }
function getLang(){ return 'ar'; }
function setLang(){ /* no-op: Arabic only */ }
function setupLanguageSelect(){ try { document.getElementById('langLi') && document.getElementById('langLi').remove(); } catch (_) {} }

var translateCatalogUi = function(key, fallback){ return fallback == null ? '' : String(fallback); };
var translateCatalogDynamicText = function(rawText){ return rawText == null ? '' : String(rawText); };
...
try {
  window.__I18N__ = {
    ...
    setupLanguageSelect: setupLanguageSelect,
    prewarm: function(){ return 0; }
  };
} catch (_) {}
try {
...
(function(){
  function arabicOnly(){
    ...
    try { localStorage.removeItem('site:lang'); } catch (_) {}
  }
  function removeLangSwitcher(){ try { var el = document.getElementById('langLi'); if (el) el.remove(); } catch (_) {} }
  function boot(){ arabicOnly(); try { enforceLatinDigits(document); } catch (_) {} removeLangSwitcher(); }
  arabicOnly();
...
//   { ok:false, unauthorized:true, code }            -> 401 (session rejected) — caller
//                                                       should re-sync, not treat as network
//   { ok:false, unavailable:true, code }             -> 503 balance_unavailable — keep cache
//   null                                             -> network failure / not authed locally
async function fetchHeaderAccountFromServer(user){
  ...
  const work = (async () => {
    try {
      // النقل الموحّد: كوكي الجلسة HttpOnly يحمل الجلسة وحده
      // (credentials:'include' يفرضه غلاف fetch في site-core).
...
function stopHeaderFirestoreForSuppressedRoute(){
  if (!isHeaderFirestoreSuppressedRoute()) return;
  try {
  ...
  try {
    const user = (typeof firebase !== 'undefined' && firebase && typeof firebase.auth === 'function')
      ? firebase.auth().currentUser
      : null;
    if (user) applyHeaderCachedUserState(user);
  } catch {}
}
try { window.addEventListener('hashchange', stopHeaderFirestoreForSuppressedRoute); } catch {}
...
try { window.addEventListener('site-state-updated', function(){ syncTransferSidebarVisibility(); }); } catch {}
...
function readHeaderSiteTransfersState(){
  var siteState = headerReadResolvedSiteState();
  var raw = siteState && typeof siteState === 'object' ? (siteState.transfers || null) : null;
  ...
  var hasExplicitEnabled =
    ...
    Object.prototype.hasOwnProperty.call(src, 'active');
  return {
    // Default ENABLED when unset so an existing working section is not hidden
    // until an admin explicitly turns it off — mirrors the backend default.
    enabled: hasExplicitEnabled
      ? !!(
        src.enabled === true ||
        src.on === true ||
...
function setTransferSidebarVisibility(enabled){
  var transferBtn = resolveSidebarNode('transferBtn', typeof transferLi !== 'undefined' ? transferLi : null);
  if (!transferBtn) return;
  // «تحويل الرصيد» is login-gated, so it only shows when the viewer is logged in
  // AND the section is enabled. We never force it visible for a logged-out user.
  var logged = false;
  try { logged = isSidebarLoggedIn(resolveEffectiveSidebarUser(window.__AUTH_LAST_USER__ || null)); } catch (_) { logged = false; }
  transferBtn.style.display = (enabled && logged) ? 'flex' : 'none';
}
function syncTransferSidebarVisibility(){
  setTransferSidebarVisibility(readHeaderSiteTransfersState().enabled !== false);
}
function readHeaderSiteAccountsState(){
  var siteState = headerReadResolvedSiteState();
  var raw = siteState && typeof siteState === 'object' ? (siteState.accountsSite || null) : null;
  var src = raw && typeof raw === 'object' ? raw : {};
  // Default DISABLED (fail-CLOSED): «موقع الحسابات» is a PAID section that stays
  // hidden until the owner buys the accounts_marketplace feature — mirrors the
  // backend readAccountsSiteEnabled (the OPPOSITE of transfers, which defaults on).
  return {
    enabled: !!(
      src.enabled === true ||
      src.on === true ||
    ...
    // Storefront card identity, set by the owner in «إعدادات موقع الحسابات» so
    // the marketplace sits among the shipping sections looking like one of them.
    name: String(src.name || src.title || '').trim(),
    imageUrl: String(src.imageUrl || src.image || '').trim(),
    // Position among the home grid's root sections (0 = first), set by dragging
    // the row in «الأقسام والمنتجات».
    sortOrder: Math.max(0, Number(src.sortOrder || src.sort_order || 0) || 0)
  };
}
try { window.__isAccountsMarketEnabled = function(){ return readHeaderSiteAccountsState().enabled === true; }; } catch {}
try {
...
try {
  window.__isSiteTransfersEnabled = function(){
    return readHeaderSiteTransfersState().enabled !== false;
  };
} catch {}
...
function applyAuthUi(user){
  ...
  if (logged) {
    ...
    setSidebarNodeVisibility(transferBtn, readHeaderSiteTransfersState().enabled !== false, 'flex');
...
// The store's configured brand name, straight from site_settings.js (GENERATED
// from store.identity.json). This is the ONLY place a default brand may come
// from — the template is deployed for several stores and a hardcoded name would
// follow the copy into the next one.
function readConfiguredStoreName(){
  try {
    return String((window.__getSiteSetting ? window.__getSiteSetting("brand.storeName", "") : "") || "").trim();
  } catch (_) {
    return "";
  }
}
function readInstallAppBrandName(){
  ...
  try {
    // Last resorts only — the configured brand, then the host. NEVER a literal
    // store name: this template ships to several stores (see store.identity.json).
    return normalizeInstallAppText(readConfiguredStoreName()) || normalizeInstallAppText(window.location && window.location.hostname) || "";
  } catch (_) {
    return normalizeInstallAppText(window.location && window.location.hostname) || "";
  }
}
...
// Once a src fails (network/CORS or tainted canvas) never re-attempt it this
// page load — retrying on every manifest rebuild spammed the network tab with
// an endless stream of failing icon fetches.
let siteInstallRasterFailedSrc = "";
...
function prepareSiteInstallRasterIcons(iconUrl){
  ...
  if (siteInstallRasterFailedSrc === src) return;
  ...
  img.onload = function(){
    ...
    if (!entries.length) { siteInstallRasterFailedSrc = src; return; } // canvas tainted (no CORS) — keep the raw-URL floor
  ...
  img.onerror = function(){ siteInstallRasterPending = ""; siteInstallRasterFailedSrc = src; };
...
function buildSiteInstallManifestPayload(){
  const name = readInstallAppBrandName() || readConfiguredStoreName();
  const shortName = normalizeInstallAppText(name).slice(0, 24) || name;
...
function setDynamicSiteManifestLink(){
  ...
  // While the payload is unchanged the PREVIOUS blob URL is reused — minting a new
  // blob per theme/brand/media apply made the browser refetch the manifest each time.
  try {
    ...
    if (payload && window.URL && typeof window.URL.createObjectURL === "function" && typeof Blob === "function") {
      ...
      if (json === __siteManifestBlobMemo.json && __siteManifestBlobMemo.href) {
        return setSiteManifestLinkHref(__siteManifestBlobMemo.href, true);
      }
      const blob = new Blob([json], { type: "application/manifest+json" });
      const blobUrl = window.URL.createObjectURL(blob);
      if (blobUrl) {
        __siteManifestBlobMemo = { json: json, href: blobUrl };
        return setSiteManifestLinkHref(blobUrl, true);
      }
    }
  } catch {}
...
function ensureSiteInstallManifest(){
  // The index.html-inline manifest system (window.__refreshDynamicSiteManifestHead)
  // owns the SAME <link rel="manifest"> element. Running both systems made them
  // ping-pong two different blob URLs on one link — an endless manifest+icon
  // refetch loop in the network tab. Delegate to the inline owner when present.
  try {
    if (typeof window.__refreshDynamicSiteManifestHead === "function"
...
function clearAuthClientState(){
  ...
  if (uid) {
    // Per-user leftovers that used to survive logout: the whole user-state
    // bundle (balance + apiAccess), TOTP material, the deposit fast-poll
    // marker, and per-uid seen-markers. The referral inviter stash is KEPT —
    // it's pre-login attribution.
    try { localStorage.removeItem('auth:user-state:v1:' + uid); } catch {}
    try { localStorage.removeItem('api:access:enabled:' + uid); } catch {}
    try { localStorage.removeItem('security:totp:secret:' + uid); } catch {}
    try { localStorage.removeItem('security:totp:pending:' + uid); } catch {}
    try { localStorage.removeItem('security:totp:enabled:' + uid); } catch {}
    try { localStorage.removeItem('deposit:pending:' + uid); } catch {}
    try { localStorage.removeItem('wallet:creditMotionSeen:' + uid); } catch {}
    try { localStorage.removeItem('edaa:referral:levelSeen:v1:' + uid); } catch {}
    try { localStorage.removeItem('edaa:referral:levelSeen:v1:' + uid + ':req'); } catch {}
    try { localStorage.removeItem('edaa:referral:earnedSeen:v1:' + uid); } catch {}
  }
  try {
...
function performClientLogout(redirectUrl){
  ...
  hideBannedOverlay();
  // The HttpOnly session cookie cannot be deleted by page script — ask the
  // server to clear it (fire-and-forget; local sign-out proceeds regardless).
  try {
...
try { syncTransferSidebarVisibility(); } catch {}
// حماية الحساب
...
function resolveHeaderExtraGapPx() {
  // يجب أن تطابق --app-header-gap في site-bundle.css، وإلا أعاد هذا السطر
  // كتابة المتغيّر بقيمة أكبر بعد الإقلاع فيعود الفراغ تحت الهيدر.
  try {
    if (window.matchMedia && window.matchMedia('(max-width: 600px)').matches) return 10;
  } catch {}
  return 12;
}
...
function writePostLoginPayload(payload){
  try {
    ...
    // The storage layer (site-core bundle) strips sessionKey/authkey on write;
    // the in-memory copy keeps the full payload for this page's runtime.
    localStorage.setItem(POST_LOGIN_STORAGE_KEY, JSON.stringify(data));
    try {
      // window.name survives same-tab navigations and is readable by any
      // same-origin page — mirror NON-SECRET display fields only.
      const mirror = (typeof window.__siteBuildTransientAuthMirror === 'function')
        ? window.__siteBuildTransientAuthMirror(data)
        : null;
      window.name = mirror ? (TRANSIENT_AUTH_PREFIX + JSON.stringify(mirror)) : '';
    } catch {}
    try { window.__POST_LOGIN_PAYLOAD__ = data; } catch {}
  } catch {}
}
...
function buildFallbackUserFromPayload(payload){
  ...
  // The persisted payload no longer carries sessionKey/authkey (secrets policy —
  // the credential lives in the HttpOnly cookie). A reasonably fresh payload is
  // enough for the DISPLAY-ONLY fallback identity; real API calls still
  // authenticate via the cookie / session info.
  const payloadAgeMs = Date.now() - Number(payload.ts || 0);
  const isFreshPayload = Number.isFinite(payloadAgeMs) && payloadAgeMs >= 0 && payloadAgeMs < 30 * 24 * 60 * 60 * 1000;
  if (!hasSession && !hasAuthKey && !isFreshPayload) return null;
  return {
    uid,
    email: payload.email || '',
    displayName,
...
// Single-flight the session sync per uid. The settings page loads the header, the
// account screen AND the wallet at once, and each independently calls this to (re)mint
// a session. In the D1 cutover there is ONE session row per uid, so N concurrent syncs
// each minted a different key and overwrote each other → the key stored in localStorage
// diverged from the one that survived in D1 → account-info 401 → "تعذر تحميل البيانات".
// De-duplicating concurrent calls means exactly ONE sync (one key) per burst, and every
// caller shares its result, so they all converge on the same session.
...
try {
  ...
  (async ()=>{
    ...
    if (user) {
      ...
      // 401 healing: the sync hands every device the account's UNIFIED session key,
      // so a rejected key is first re-synced (one attempt per minute; the sync is
      // single-flighted per uid) and only a rejection that SURVIVES a fresh sync is
      // surfaced as a real session end. Returns true when resolved (data applied /
      // ban handled / logout surfaced); false = possible network blip, keep cache.
      let lastSessionResyncAt = 0;
      const handleUnauthorizedAccountInfo = async () => {
        if (Date.now() - lastSessionResyncAt < 60000) return false;
        lastSessionResyncAt = Date.now();
        let synced = null;
        try {
          const idToken = await user.getIdToken().catch(() => '');
          if (!idToken) return false;
          synced = await syncCatalogAuthFromToken(idToken, readPostLoginPayload() || {}).catch(() => null);
        } catch { synced = null; }
        if (!synced || !synced.sessionKey) return false;
        const retried = await fetchHeaderAccountFromServer(user).catch(() => null);
        if (retried && retried.banned) {
          markBannedSessionUid(user.uid);
          handleBannedAccount(retried.banReason, retried.webuid || user.uid || '', user.uid);
          return true;
        }
        if (retried && retried.ok) {
          applyHeaderServerInfo(retried);
          return true;
        }
        if (retried && retried.unauthorized) {
          // The sync just handed us the account's current key and the server STILL
          // rejects it → the session is definitively gone (revoked / rotated away).
          // Surface a clean logout instead of the silent 0-balance limbo.
          triggerSessionConflictLogout('session_expired');
          return true;
        }
        return false;
      };
      for (let attempt = 0; attempt < 4 && !headerLoadedFromServer; attempt++) {
        ...
        if (headerServerInfo && headerServerInfo.banned) {
          ...
          if (await handleUnauthorizedAccountInfo()) {
            headerLoadedFromServer = true;
          } else if (attempt < 3) {
            await new Promise(resolve => setTimeout(resolve, 500 + attempt * 400));
          }
        } else if (headerServerInfo && headerServerInfo.unavailable) {
          // Authoritative answer: balance temporarily unreadable (Neon down, no
          // mirror). Keep the cached balance; retrying now would not help.
          break;
        } else if (attempt < 3) {
      ...
      // Register the per-user throttled refresher consumed by the module-level
      // focus/interval/deposit-poll triggers (single-flight + 15s min gap).
      window.__headerBalanceRefresh = (reason) => {
        try {
          if (__headerBalanceRefreshInflight) return __headerBalanceRefreshInflight;
          if (Date.now() - __headerBalanceLastRefreshAt < HEADER_BALANCE_REFRESH_MIN_INTERVAL_MS) return null;
          __headerBalanceLastRefreshAt = Date.now();
          __headerBalanceRefreshInflight = (async () => {
            try {
              const refreshed = await fetchHeaderAccountFromServer(user).catch(() => null);
              if (refreshed && refreshed.banned) {
                markBannedSessionUid(user.uid);
                handleBannedAccount(refreshed.banReason, refreshed.webuid || user.uid || '', user.uid);
                return true;
              }
              if (refreshed && refreshed.ok) {
                applyHeaderServerInfo(refreshed);
                return true;
              }
              if (refreshed && refreshed.unauthorized) {
                return await handleUnauthorizedAccountInfo();
              }
              // unavailable (503) or network failure: keep the cached balance.
              return false;
            } finally {
              __headerBalanceRefreshInflight = null;
            }
          })();
          return __headerBalanceRefreshInflight;
        } catch { return null; }
      };
      try { resumeDepositPendingPollIfFresh(user.uid); } catch {}
    } else {
      ...
      setHeaderBalanceAmount(0);
      broadcastBalance(0);
      try { window.__headerBalanceRefresh = null; } catch {}
      try { stopDepositPendingPoll(); } catch {}
    }
...
// ---- Server balance refresher ----
// Deposit credits land in Neon/D1 server-side with no push channel to this
// client (the old Firestore onSnapshot listener is gone), so the header re-asks
// the server on focus/visibility, on a gentle visible-tab interval, on entering
// money routes, and in a bounded fast poll right after the customer submits a
// deposit. All triggers funnel through window.__headerBalanceRefresh (registered
// per-user by the auth boot) behind a single-flight + 15s min-gap throttle.
const HEADER_BALANCE_REFRESH_MIN_INTERVAL_MS = 15000;
const HEADER_BALANCE_IDLE_REFRESH_MS = 60000;
const DEPOSIT_PENDING_POLL_MS = 20000;
const DEPOSIT_PENDING_POLL_WINDOW_MS = 10 * 60 * 1000;
let __headerBalanceRefreshInflight = null;
let __headerBalanceLastRefreshAt = 0;
let __depositPendingPollTimer = null;

function depositPendingStorageKey(uid){ return 'deposit:pending:' + String(uid || '').trim(); }

function currentHeaderRefreshUid(){
  try { return String(localStorage.getItem(LAST_UID_KEY) || '').trim(); } catch { return ''; }
}

function readDepositPendingEntry(uid){
  try {
    const parsed = JSON.parse(localStorage.getItem(depositPendingStorageKey(uid)) || 'null');
    return (parsed && typeof parsed === 'object') ? parsed : null;
  } catch { return null; }
}

function clearDepositPendingEntry(uid){
  try { localStorage.removeItem(depositPendingStorageKey(uid)); } catch {}
}

function stopDepositPendingPoll(){
  if (__depositPendingPollTimer) {
    try { clearInterval(__depositPendingPollTimer); } catch {}
    __depositPendingPollTimer = null;
  }
}

function startDepositPendingPoll(){
  if (__depositPendingPollTimer) return;
  __depositPendingPollTimer = setInterval(() => {
    try {
      const uid = currentHeaderRefreshUid();
      const entry = uid ? readDepositPendingEntry(uid) : null;
      if (!entry || !Number(entry.at) || (Date.now() - Number(entry.at)) > DEPOSIT_PENDING_POLL_WINDOW_MS) {
        if (uid) clearDepositPendingEntry(uid);
        stopDepositPendingPoll();
        return;
      }
      const startBal = Number(entry.bal);
      const nowBal = Number(window.__BAL_BASE__);
      if (Number.isFinite(startBal) && Number.isFinite(nowBal) && nowBal > startBal + 1e-9) {
        clearDepositPendingEntry(uid);
        stopDepositPendingPoll();
        return;
      }
      const fn = window.__headerBalanceRefresh;
      if (typeof fn === 'function') fn('deposit-poll');
    } catch {}
  ...
  try {
    ...
    if (uid) {
      ...
      localStorage.setItem(
        ...
        JSON.stringify({ at: Date.now(), bal: Number.isFinite(bal) ? bal : null })
      );
    }
  } catch {}
...
(function setupHeaderBalanceRefreshTriggers(){
  const kick = (reason) => {
    try {
      const fn = window.__headerBalanceRefresh;
  ...
  try { window.addEventListener('focus', () => kick('focus')); } catch {}
  try {
    window.addEventListener('hashchange', () => {
      if (/^#\/(wallet|orders|dafaati)(\/|$)/.test(String(location.hash || ''))) kick('route');
    });
  } catch {}
  try {
    setInterval(() => {
      if (document.visibilityState === 'visible') kick('interval');
    }, HEADER_BALANCE_IDLE_REFRESH_MS);
  } catch {}
  try { window.addEventListener('store:deposit-submitted', () => markDepositPendingAndPoll()); } catch {}
})();
  ...
  try {
    ...
    const normalizeDockRouteShortcut = (value) => {
      ...
      try {
        ...
        // فرع الـhash يُستشار فقط إن وُجد hash فعلًا: `resolveAlias('')` يُرجع
        // '#/' (الجذر)، فكانت أي قيمة بلا hash — مثل "wallet" التي ترسلها لوحة
        // الأدمن — تنطوي إلى الرئيسية قبل أن يُقرأ المسار أصلًا.
        const hashText = String(url.hash || '').trim();
        if (hashText) return resolveAlias(hashText) || '';
    ...
    const normalizeDock = () => {
      ...
      return {
        ...
        items: defaultItems.map((fallback, index) => {
          ...
          // وجهة الزر هي مفتاحه (تمامًا كالشريط الجانبي)؛ الـhref المخزَّن لا
          // يتجاوزها إلا إن أشار لمكان آخر فعلًا. الإعدادات المحفوظة سابقًا
          // صارت كلها "#/" بسبب طيّ المُطبِّع للقيم بلا hash، فكان كل زر يعيد
          // للرئيسية — لذلك href الجذر على مفتاح غير الرئيسية يُعامَل كغير مضبوط.
          const keyRoute = normalizeDockRouteShortcut(key) || normalizeDockHref(def.route || def.href, '#/') || '#/';
          let href = normalizeDockHref(explicitHref, keyRoute);
          if (!href || (href === '#/' && key !== 'home')) href = keyRoute;
          ...
          return {
            key,
            // «مخصّص» = وجهة خارج مسار المفتاح؛ العناصر المخصّصة وحدها تتخطّى
            // بوابة الظهور (isItemVisible)، فلا يجوز أن تُصنَّف كذلك لمجرد أن
            // الـhref المخزَّن كان يساوي مسار مفتاحها.
            custom: explicitHref != null && String(explicitHref).trim() !== '' && href !== keyRoute && href !== (fallback.href || def.href),
    ...
    function isItemVisible(key, user){
      ...
      if (key === 'transfer') {
        try { if (readHeaderSiteTransfersState().enabled === false) return false; } catch {}
      }
      if (key === 'deposit') {
        const brand = readHeaderSiteBrandState();
    ...
    function getDockRouteKeyFromHash(hash, fallbackKey){
      try {
        ...
        // مسار الجذر = الرئيسية دائمًا، لا مفتاح الزر: تمرير المفتاح هنا كان
        // يجعل الوجهة والمفتاح متناقضين (الرابط '#/' والمسار 'wallet') فيُرسم
        // مسار غير الذي يعبّر عنه الرابط ثم يُعيده مُنقذ القشرة للرئيسية.
        const key = String(first || (raw.trim() ? fallbackKey : 'home') || '').trim().toLowerCase();
    ...
    function navigateDockRoute(targetHash, key){
      ...
      try {
        ...
        // ملاحة مطابقة للشريط الجانبي: نداء واحد لـnavigateHomeHash وكفى.
        // إعادة الضبط المؤجّلة (setTimeout 0) التي كانت هنا كانت تستدعي
        // __reloadInlineRoute فترسم مسارًا لا يعبّر عنه الرابط، ثم يعيد
        // مُنقذ القشرة المستخدمَ إلى الصفحة التي انطلق منها.
    ...
    function handleDockItemClick(ev){
      try {
        ...
        // نقرة واحدة على الجوال تُطلق pointerup ثم touchend ثم click، والثلاثة
        // مربوطة بنفس المعالج؛ كان الكبح يشمل click فقط فتنفَّذ الملاحة مرتين
        // وتُسجَّل قيدان في السجل. الكبح الآن لكل أنواع التفعيل.
        const now = Date.now();
        if (lastDockActivationAt && now - lastDockActivationAt < 450) {
          if (ev) {
            ev.preventDefault();
            ev.stopPropagation();
...
(function(){
  try {
    ...
    // -----------------------------------------------------------------------
    // مكدّس الأزرار العائمة (أسفل يسار) — قاعدة واحدة عامة بدل إزاحات ثابتة.
    //
    // الترتيب من الأسفل للأعلى: قائمة الدعم «...» ← زر شات الدعم ← اختصار
    // واتساب. كل زر يجلس فوق الذي تحته بارتفاعه **المقيس فعليًا**، وقاعدة
    // المكدّس ترتفع بارتفاع الشريط السفلي **إن كان ظاهرًا حقًّا** فقط.
    //
    // ولذلك: إن غاب الشريط السفلي نزل الجميع إلى حافة الشاشة بلا فراغ، وإن
    // غاب أي زر (مخفي، ممنوع بالصلاحية، أو غير موجود أصلًا) أخذ من فوقه
    // مكانه تلقائيًا. لا حاجة لتعديل أي رقم عند إضافة زر أو حذفه — يكفي
    // إدراجه في FLOAT_STACK_ORDER.
    // -----------------------------------------------------------------------
    const FLOAT_STACK_GAP = 12;   // بين زر وآخر، وبين أول زر والشريط السفلي
    const FLOAT_STACK_EDGE = 14;  // بين أول زر وحافة الشاشة حين لا شريط سفلي

    function floatStackVisibleHeight(el){
      ...
      // offsetHeight لا يتأثر بتحويلات الأنيميشن (scale) بعكس
      // getBoundingClientRect الذي يلتقط الارتفاع المصغّر أثناء الحركة.
      let rectHeight = 0;
      try { rectHeight = Math.ceil(el.getBoundingClientRect().height || 0); } catch(_){}
      const height = Math.max(Number(el.offsetHeight) || 0, rectHeight);
      return height > 0 ? height : 0;
    }

    const FLOAT_STACK_ORDER = [
      {
        id: 'supportFloatingWidget',
        measure: (el) => {
          // القائمة المغلقة تحتفظ بارتفاع عناصرها في التخطيط (تُخفى بـopacity
          // وtransform لا بـdisplay)، فالمساحة المحجوزة فعلًا هي الزرّ وحده
          // ما لم تكن مفتوحة.
          if (el.classList && el.classList.contains('is-open')) return floatStackVisibleHeight(el);
          const toggle = el.querySelector ? el.querySelector('.support-dock__toggle') : null;
          return floatStackVisibleHeight(toggle || el);
        }
      },
      { id: 'siteSupportChatFab' },
      { id: 'waJoinShortcutButton' }
    ];

    function layoutFloatingStack(){
      try {
        const dock = document.querySelector('.mobile-dock');
        const dockHeight = floatStackVisibleHeight(dock);
        // صندوق الشريط السفلي يغطي أصلًا هامش الأمان السفلي (padding داخله)،
        // فلا يُضاف مرتين. بلا شريط نضيف env مباشرة.
        const base = dockHeight
          ? (dockHeight + FLOAT_STACK_GAP) + 'px'
          : 'calc(' + FLOAT_STACK_EDGE + 'px + env(safe-area-inset-bottom, 0px))';
        let offset = 0;
        FLOAT_STACK_ORDER.forEach((entry) => {
          const el = document.getElementById(entry.id);
          if (!el) return;
          const height = entry.measure ? entry.measure(el) : floatStackVisibleHeight(el);
          if (!height) {
            try { el.style.removeProperty('--float-stack-bottom'); } catch(_){}
            return;
          }
          const value = offset ? 'calc(' + base + ' + ' + offset + 'px)' : base;
          try { el.style.setProperty('--float-stack-bottom', value); } catch(_){}
          offset += height + FLOAT_STACK_GAP;
        });
        try {
          window.dispatchEvent(new CustomEvent('support-floating-stack-updated', {
            detail: { dockHeight: dockHeight, stackHeight: offset }
          }));
        } catch(_){}
      } catch(_){}
    }

    try { window.__layoutFloatingStack = layoutFloatingStack; } catch(_){}

    function refreshWaJoinSeparateLift(){
      layoutFloatingStack();
    }

    // إعادة الترتيب عند أي تغيّر يمسّ ظهور الشريط السفلي أو أحد الأزرار:
    // تغيّر المقاس، أو أصناف body/html التي تخفي الشريط، أو إضافة/حذف زر.
    (function watchFloatingStack(){
      let scheduled = false;
      const schedule = function(){
        if (scheduled) return;
        scheduled = true;
        const run = function(){ scheduled = false; layoutFloatingStack(); };
      ...
      try {
        if (typeof ResizeObserver !== 'undefined') {
          ...
          const observeById = function(node){ if (node) { try { ro.observe(node); } catch(_){} } };
          const attach = function(){
            observeById(document.querySelector('.mobile-dock'));
            FLOAT_STACK_ORDER.forEach((entry) => observeById(document.getElementById(entry.id)));
          };
          attach();
          // الشريط وزر الشات يُبنيان لاحقًا (حزمة الدعم كسولة) — نعيد الربط.
          window.addEventListener('support-floating-stack-attach', attach);
      ...
      try {
        if (typeof MutationObserver !== 'undefined') {
          ...
          const host = document.body || document.documentElement;
          if (host) {
            mo.observe(host, { attributes: true, attributeFilter: ['class', 'data-inline-route'], childList: true });
          }
      ...
      try {
        if (!isWaJoinShortcutVisible('separate') || !isSupportFloatingAllowed(supportFloatingAuthUser)) {
          if (waJoinSeparateButton) {
            ...
            waJoinSeparateButton.style.removeProperty('--float-stack-bottom');
          }
          layoutFloatingStack();
          return;
        }
    ...
    floatingStyle.textContent = `
      #supportFloatingWidget {
        ...
        /* الموضع يحسبه layoutFloatingStack؛ القيمة هنا احتياط أول رسم فقط. */
        bottom: var(--float-stack-bottom, calc(14px + env(safe-area-inset-bottom, 0px)));
        z-index: 9300;
        display: flex;
        flex-direction: column;
        align-items: center;
      ...
      #waJoinShortcutButton {
        ...
        bottom: var(--float-stack-bottom, calc(82px + env(safe-area-inset-bottom, 0px)));
        z-index: 9298;
        width: 46px;
        height: 46px;
        border-radius: 50%;
      ...
      @media (max-width: 640px) {
        #supportFloatingWidget {
          ...
          bottom: var(--float-stack-bottom, calc(70px + env(safe-area-inset-bottom, 0px)));
          z-index: 9100;
          gap: 10px;
        }
        #waJoinShortcutButton {
          left: max(14px, calc(env(safe-area-inset-left, 0px) + 14px));
          bottom: var(--float-stack-bottom, calc(134px + env(safe-area-inset-bottom, 0px)));
          z-index: 9098;
          width: 44px;
          height: 44px;
        }
...
// ---- Support chat (lazy) ----
// The whole chat subsystem ships as chunk-support.js (see src-frontend/support-chat.js).
// Pulled in on first user interaction or after a short idle delay — whichever
// comes first — so it stays off the critical parse path while badge polling /
// visibility behavior remain effectively unchanged once mounted.
(function(){
  try {
    if (window.__siteSupportChatLoaderInstalled) return;
    window.__siteSupportChatLoaderInstalled = true;
    var kicked = false;
    function loadSupportChunk(){
      if (kicked) return;
      kicked = true;
      try {
        if (typeof window.__loadRouteChunk === 'function') {
          window.__loadRouteChunk('__support').catch(function(){ kicked = false; });
        }
      } catch (_) { kicked = false; }
    }
    ...
    function ensureCss(){
      ...
      st.textContent = `
        #maintenance-overlay{
          ...
          align-items:center;
          justify-content:center;
          padding:calc(var(--app-header-offset, var(--app-header-height, 56px)) + 36px) 24px 64px;
        ...
        @media (max-width: 720px){
          #maintenance-overlay{
            padding:calc(var(--app-header-height, 56px) + 28px) 16px 48px;
          }
          #maintenance-overlay .maintenance-logo{
            width:128px;
            height:128px;
...
.catalog-inline-host #catalogOffersContainer,
...
`;
    ...
    function buildHeaderSeoStoreTitle(value){
      ...
      return normalizeSiteStoreNameValue(value, DEFAULT_SITE_STORE_NAME) || DEFAULT_SITE_STORE_NAME || readConfiguredStoreName();
    }
    ...
    function preloadImageAsset(url, opts){
      const src = normalizeSiteMediaUrl(url);
      if (!src) return;
      ...
      // Only the LCP candidate (the first hero banner) is warmed eagerly. Every
      // other asset — extra banners, the loader logo, the site icon, theme scenes —
      // is warmed at LOW priority on idle. A <link rel=preload as=image> or an
      // eager fetch here forces a high-priority download; doing that for a dozen
      // full-resolution images at once saturates a throttled connection and starves
      // the actual LCP image (the root cause of the 38s LCP on slow networks).
      const eager = !!(opts && opts.eager);
      const warm = function(){
        try {
          if (eager && document.head && !document.querySelector("link[rel='preload'][as='image'][href='" + src.replace(/'/g, "\\'") + "']")) {
            const ln = document.createElement("link");
            ln.rel = "preload";
            ln.as = "image";
            ln.href = src;
            try { ln.fetchPriority = "high"; } catch {}
        ...
        try {
          ...
          try { img.fetchPriority = eager ? "high" : "low"; } catch {}
          img.src = src;
        } catch {}
      };
      if (eager) { warm(); return; }
      try {
    ...
    function applyHeroBanners(list){
      ...
      // Warm only the first banner (the LCP candidate) eagerly; the rest are
      // idle/low-priority so they don't contend with it. The slider itself
      // renders slide 0 eager and the others lazy, so this only mirrors that.
      next.forEach((item, i) => preloadImageAsset(item.image, { eager: i === 0 }));
    }
