...
`;
  ...
  // تعريفان محليان وجوباً — كانا يعيشان في قسم صفحة الطلبات (calendar-inline)
  // أيام الحزمة الواحدة، وبعد التقسيم صار ذلك القسم جزءاً كسولاً بنطاق منفصل
  // لا يُحمَّل في صفحة المحفظة أصلاً. استدعاء escapeHtml العاري كان يرمي
  // ReferenceError عند رسم أول حركة حقيقية فيقتل loadWalletFor قبل سطر إخفاء
  // اللودر (لودر أبدي بلا أي أثر — الـcatch الخارجي يبتلعه)؛ ولم يظهر قبلاً
  // لأن buildTransactionHTML لا تُستدعى إلا بوجود حركات. formatOrderDurationMs
  // كانت داخل try محلي فتُسقط «مدة التنفيذ» بصمت في نفس المسار.
  ...
  async function initWalletLikePage(pageMode){
    ...
    (function(auth, db, pageConfig){
      ...
      async function fetchWalletTransactionsFromServer(uid, options){
        ...
        // (credentials:'include' يفرضه غلاف fetch) — لا ترويسة ولا رابط.
        var requestUrl = buildClientWalletRequestUrl(safeUid, options || {});
