// Static storefront settings are loaded from site_settings.js.
var SITE_CORE_SETTINGS = (typeof window !== "undefined" && window.__SITE_SETTINGS__ && typeof window.__SITE_SETTINGS__ === "object")
  ? window.__SITE_SETTINGS__
  : {};
var SITE_CORE_FIREBASE_FRONTEND_CONFIG = Object.assign({}, SITE_CORE_SETTINGS.firebase || {});
var SITE_CORE_WORKERS_CONFIG = Object.assign({}, SITE_CORE_SETTINGS.workers || {});
var SITE_CORE_BRAND_CONFIG = Object.assign({}, SITE_CORE_SETTINGS.brand || {});
var SITE_CORE_CANONICAL_API_BASE = String(SITE_CORE_WORKERS_CONFIG.routerBase || "").replace(/\/+$/, "");
var SITE_CORE_CANONICAL_API_HOST = (function () {
  try { return SITE_CORE_CANONICAL_API_BASE ? new URL(SITE_CORE_CANONICAL_API_BASE).hostname.toLowerCase() : ""; }
  catch (_) { return ""; }
})();
var SITE_CORE_ROUTER_HOST_ALIASES = Array.isArray(SITE_CORE_WORKERS_CONFIG.routerHostAliases)
  ? SITE_CORE_WORKERS_CONFIG.routerHostAliases.map(function (host) { return String(host || "").trim().toLowerCase(); }).filter(Boolean)
  : [];
...
(function (global) {
  ...
  function installAuthSessionStorageBundle() {
    try {
      ...
      function parseCountriesStorageKey(key) {
        var match = /^edaa:(deposit):countries(?::(meta))?:v3:(.+)$/i.exec(safeKey(key));
      ...
      // Secrets policy (2026-07-19): the PERSISTED postLoginPayload must never
      // carry the session key or the retired authkey — the session credential
      // lives in the كوكي الجلسة HttpOnly cookie (plus transitional
      // sessionKeyInfo until frontend R2). idToken/customToken stay: they
      // self-expire and power the Firebase re-bootstrap that keeps the user
      // signed in across browser restarts. Enforced HERE at the storage
      // interception layer so no writer — present or future — can regress it.
      var POST_LOGIN_SECRET_FIELDS = ["sessionKey", "session_key", "authkey", "authKey"];
      function stripPostLoginSecrets(payload) {
        if (!payload || typeof payload !== "object") return payload;
        var changed = false;
        for (var i = 0; i < POST_LOGIN_SECRET_FIELDS.length; i += 1) {
          if (Object.prototype.hasOwnProperty.call(payload, POST_LOGIN_SECRET_FIELDS[i])) { changed = true; break; }
        }
        if (!changed) return payload;
        var out = Object.assign({}, payload);
        POST_LOGIN_SECRET_FIELDS.forEach(function (field) {
          try { delete out[field]; } catch (_) {}
        });
        return out;
      }
      try { global.__siteStripPostLoginSecrets = stripPostLoginSecrets; } catch (_) {}
      ...
      function compactBundle(source) {
        ...
        if (hasObjectValues(src.postLoginPayload)) out.postLoginPayload = stripPostLoginSecrets(src.postLoginPayload);
  ...
  function runLocalStorageMaintenance() {
    try {
      ...
      var staleCountriesPattern = /^edaa:(?:deposit):countries(?::meta)?:v[12]$/;
      // Old catalog cache generations (the prefix moved to v9 without a sweep).
      var staleCatalogPattern = /^catalog:(?:cache|section):v[1-8]:/;
      for (var i = storage.length - 1; i >= 0; i -= 1) {
        var key = "";
        try { key = String(storage.key(i) || ""); } catch (_) { key = ""; }
        if (!key) continue;
        if (staleCountriesPattern.test(key) || staleCatalogPattern.test(key)) {
          try { storage.removeItem(key); } catch (_) {}
        }
      }
      // Legacy bare `theme` key: superseded by site:theme:v1 (appearance
      // bundle). Only dropped once the bundle copy exists — readers that still
      // prefer the legacy key fall back to the bundle either way.
      try {
        if (storage.getItem("theme") != null && storage.getItem("site:theme:v1") != null) {
  ...
  // One-time scrub for bundles persisted BEFORE the secrets policy: any stored
  // postLoginPayload still carrying sessionKey/authkey gets re-written through
  // the (stripping) virtual layer on this boot.
  (function scrubLegacyPersistedSecrets() {
    try {
      var storage = global.localStorage;
      if (!storage) return;
      var raw = storage.getItem("postLoginPayload");
      if (!raw) return;
      if (!/"(?:sessionKey|session_key|authkey|authKey)"\s*:/.test(raw)) return;
      storage.setItem("postLoginPayload", raw);
    } catch (_) {}
  })();

  // ---- Credentialed API fetch (HttpOnly session cookie) ----
  // The API origin sets the كوكي الجلسة HttpOnly cookie; the browser only
  // attaches it to these CROSS-ORIGIN (same-site) requests when the fetch is
  // credentialed. One patch here — site-core evaluates before every other
  // section and before any network call — covers every caller, including the
  // lazy chunks and the deposit blob scripts (same window). header.js's
  // session guard captures window.fetch AFTER this runs, so the chain holds.
  function installApiCredentialsFetchPatch() {
    try {
      if (!global.fetch || global.__API_CREDENTIALS_FETCH_PATCHED__) return;
      global.__API_CREDENTIALS_FETCH_PATCHED__ = true;
      var apiHosts = {};
      try { if (SITE_CORE_CANONICAL_API_HOST) apiHosts[SITE_CORE_CANONICAL_API_HOST] = true; } catch (_) {}
      try {
        SITE_CORE_ROUTER_HOST_ALIASES.forEach(function (host) { if (host) apiHosts[host] = true; });
      } catch (_) {}
      var nativeFetch = global.fetch.bind(global);
      function isApiTarget(value) {
        try {
          var base = (global.location && global.location.href) || undefined;
          return !!apiHosts[new URL(String(value), base).hostname.toLowerCase()];
        } catch (_) { return false; }
      }
      // عدّاد «طلبات API معلّقة» — غذاء حارس اللودر في site-startup: ما دام
      // العدّاد صفراً ولم يستقر طلب حديثاً، فأي لودر ظاهر عالق بحاجز مسرّب
      // (عدّاد كاتلوج غير متوازن، علم __CATALOG_INLINE_LOADING__ منسي…) لا
      // بتحميل حقيقي. التتبع هنا حصراً لأن هذا الغلاف يسبق كل قسم وكل نداء.
      function trackApiFetchPending(promise) {
        try {
          global.__API_FETCH_PENDING_COUNT__ = Math.max(0, Number(global.__API_FETCH_PENDING_COUNT__ || 0)) + 1;
        } catch (_) {}
        var settle = function () {
          try {
            global.__API_FETCH_PENDING_COUNT__ = Math.max(0, Number(global.__API_FETCH_PENDING_COUNT__ || 0) - 1);
            global.__API_FETCH_LAST_SETTLED_AT__ = Date.now();
          } catch (_) {}
        };
      ...
      global.fetch = function (input, init) {
        try {
          var target = (input && typeof input === "object" && typeof input.url === "string") ? input.url : input;
          if (isApiTarget(target)) {
            if (input && typeof input === "object" && typeof input.url === "string" && init == null) {
              return trackApiFetchPending(nativeFetch(new Request(input, { credentials: "include" })));
            }
            var nextInit = Object.assign({}, init || {});
            nextInit.credentials = "include";
            return trackApiFetchPending(nativeFetch(input, nextInit));
          }
        } catch (_) {}
  ...
  // Non-secret subset of the post-login payload for the window.name same-tab
  // mirror (window.name survives navigations and is readable by any same-origin
  // page — credentials must never land there).
  function buildTransientAuthMirror(payload) {
    var src = (payload && typeof payload === "object") ? payload : {};
    var out = {};
    ["uid", "email", "displayName", "name", "username", "accountNo", "deviceId", "ts"].forEach(function (field) {
      if (src[field] != null && src[field] !== "") out[field] = src[field];
    });
    return out;
  }
  try { global.__siteBuildTransientAuthMirror = buildTransientAuthMirror; } catch (_) {}
  ...
  function normalizeHttpBase(value) {
    ...
    try {
      ...
      if (SITE_CORE_CANONICAL_API_HOST && SITE_CORE_ROUTER_HOST_ALIASES.indexOf(hostname) !== -1) hostname = SITE_CORE_CANONICAL_API_HOST;
      var canonicalHost = (SITE_CORE_CANONICAL_API_HOST && /^api\./i.test(hostname) && hostname !== SITE_CORE_CANONICAL_API_HOST && /\.(?:shop|store)$/i.test(hostname))
  ...
  function appendRuntimeConfigUrls(list, seen, base) {
    ...
    try {
      var parsed = new URL(normalizedBase);
      // Runtime config is fetched only from its dedicated path endpoints; the
      // "?mode=site-runtime-config" query variant was removed so the API base
      // "?mode=" is used solely for siteState.
...
function navigateHomeHash(targetHash, routeKey){
  ...
  var isWalletFlowRoute = (keyLower === "deposit" || keyLower === "edaa");
  try {
...
async function initFirebaseApp(){
  try {
    ...
    var defaultApp = null;
    try { defaultApp = firebase.app ? firebase.app() : null; } catch (_) { defaultApp = null; }
    if (!defaultApp){
      try {
