...
(function (global) {
  ...
  function installAuthSessionStorageBundle() {
    try {
      ...
      // lives in the njad_sess HttpOnly cookie (plus transitional
  ...
  // api.njad.store sets the njad_sess HttpOnly cookie; the browser only
        ...
        try {
          ...
          if (isApiTarget(target)) {
            if (input && typeof input === "object" && typeof input.url === "string" && init == null) {
              return nativeFetch(new Request(input, { credentials: "include" }));
            }
