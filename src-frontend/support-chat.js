...
(function(){
  try {
    ...
    function getSupportFetchCredentials(requestUrl){
      // The API host is cross-origin but SAME-SITE; the كوكي الجلسة HttpOnly
    ...
    async function buildSupportAuthPayload(){
      ...
      if (typeof user.getIdToken === 'function') {
        try { idToken = String(await user.getIdToken(false) || '').trim(); }
          ...
          try { idToken = String(await user.getIdToken(true) || '').trim(); } catch (_2) { idToken = ''; }
        }
      }
      // idToken اختياري (الجلسة الموحّدة): بدونه يصادق الخادم عبر
      // X-SessionKey / كوكي الجلسة — نفس عقد الشراء. لا ترمِ هنا.
    ...
    async function callSupportApi(mode, options){
      ...
      // The session key never rides the URL (CF logs keep query strings);
      // it travels in the X-SessionKey header + the كوكي الجلسة HttpOnly cookie.
      ...
      var send = async function(nextAuth){
        ...
        // النقل الموحّد: كوكي الجلسة HttpOnly يحمل الجلسة وحده (يركب عبر
        // credentials:'include') — لا مفتاح في ترويسة ولا جسم. جسر auth فقط
        // (refreshSupportSessionOnce) يقدّم المفتاح صراحةً.
        var headers = { 'Content-Type': 'application/json' };
        if (nextAuth.idToken) headers['Authorization'] = 'Bearer ' + nextAuth.idToken;
    ...
    async function uploadSupportImageFile(file){
      ...
      var send = async function(nextAuth){
        ...
        var uploadHeaders = { 'Content-Type': 'application/json' };
        if (nextAuth.idToken) uploadHeaders['Authorization'] = 'Bearer ' + nextAuth.idToken;
        var response = await fetch(requestUrl, {
          method: 'POST',
          cache: 'no-store',
          credentials: getSupportFetchCredentials(requestUrl),
    ...
    async function uploadSupportVoiceBlob(blob){
      ...
      var send = async function(nextAuth){
        var requestUrl = getSupportEndpoint('user-upload-image');
        var voiceHeaders = { 'Content-Type': 'application/json' };
        if (nextAuth.idToken) voiceHeaders['Authorization'] = 'Bearer ' + nextAuth.idToken;
        var response = await fetch(requestUrl, {
          method: 'POST',
          cache: 'no-store',
          credentials: getSupportFetchCredentials(requestUrl),
    ...
    function supportRealtimeWsUrl(auth){
      // The DO upgrade validates the Firebase idToken; a session-only user has
      // none, so skip WS entirely — polling remains the correctness fallback.
      ...
      // in URLs); the same-site كوكي الجلسة HttpOnly cookie rides the handshake,
    ...
    function mountSupportChat(){
      ...
      style.textContent = `
        #siteSupportChatFab{
          ...
          /* الموضع يحسبه window.__layoutFloatingStack؛ ما هنا احتياط أول رسم. */
      ...
      // زر الشات يُبنى من حزمة كسولة بعد الإقلاع، فيُعاد حساب مكدّس الأزرار
      // العائمة ويُربط المراقب بالزر الجديد كي يأخذ دوره في الترتيب.
      try { window.dispatchEvent(new CustomEvent('support-floating-stack-attach')); } catch(_){}
      try { if (typeof window.__layoutFloatingStack === 'function') window.__layoutFloatingStack(); } catch(_){}
